#!/usr/bin/env python3
"""The species extraction must refuse a directory that is not exactly the registry's.

THE DEFECT (review finding A5). The embedded ROOT projector logs `PROJ_ERROR`
and CONTINUES when a pair file cannot be opened, and the Python reader required
only the eventual `PROJ_DONE` marker. It never checked WHICH files were
projected, nor how many succeeded.
...
Also covered: a mistyped `--decay-map` used to skip the experiment-comparable
grouping silently, producing a successful map-free extraction that looked
complete.
