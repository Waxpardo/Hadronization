#!/usr/bin/env python3
"""Prevent `make check` from certifying an unusable runtime.

An observed run passed 30 tests with ROOT 6.38.04 instead of pinned 6.30.01.
That host also lacked PYTHIA and CVMFS, while doctor reported two blockers.
...
# fails". Stub a `root`/`root-config`/`pythia8-config` matching both pins.
...
if want:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        r = run_verdict(path_prefix=str(stub))
        out = r.stdout + r.stderr
        check("a wrong PYTHIA version fails the exact pin",
              r.returncode != 0
              and "PYTHIA is 8.316, pinned is 8.317" in out,
              f"rc={r.returncode} {out[-300:]}")
