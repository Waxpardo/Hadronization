#!/usr/bin/env python3
"""The exit status of decompose_with_block_sems.py must report I2, not only I3.

THE DEFECT (review finding A12). The tool ran both integrity checks but
returned `0 if not mism else 3` -- I3 only. `flags_total` was accumulated and
discarded. A driver reads the exit status, so the tool could report hundreds of
I2 block-vs-central failures and still be recorded as a clean run.

WHY THIS IS NOT FIXED BY "JUST FAIL ON I2". I2's binomial null is known to be
...
Check 2 is the regression: against the pre-fix tool the same fixture exits 0.
"""
import csv
import subprocess
import sys
