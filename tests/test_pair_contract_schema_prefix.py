#!/usr/bin/env python3
"""The contract key prefix must be resolved from the config, and must not lie.

WHAT THIS TEST ESTABLISHES. That both implementations of the contract parser --
...
WHAT THIS TEST CANNOT ESTABLISH, stated plainly because the acceptance test
originally asked for is not available: **that a full plotting run is unchanged.**
...
`SUBSAMPLES_700/...` are absent locally and on Nikhef
(`docs/PLOTTING_V3_DELTA.md` §2b). No test in this repository can prove the old
figures are unchanged, because the inputs that made them are gone. This is the
agreed data-free substitute, not a proof of run-level equivalence.

Both directions are covered, as ruled:
