#!/usr/bin/env python3
"""The harvest driver must pass MONASH and must fail the ways closure can fail.

A DRIVER THAT CANNOT REPRODUCE A KNOWN TUNE IS NOT A DRIVER. So the positive
case here is MONASH's REAL recorded closure line, taken verbatim from
`docs/MONASH_CENTRAL_TABLE.md` §1, plus the committed MONASH anchors run through
the decomposition stage and required to reproduce the recorded values.
...
calls that a pass. Review finding A4 added a required expected-schema argument
to the wrapper, but that fix is local-only -- the frozen Nikhef tree the merge
reads does not have it -- so for the upcoming harvest the enforcement is this
function, and it had better fail.
"""
import subprocess
import sys
import tempfile
