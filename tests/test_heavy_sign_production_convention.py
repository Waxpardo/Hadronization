#!/usr/bin/env python3
"""`heavy_flavour_sign` must match PRODUCTION, not merely itself.

THE DEFECT (review finding A11). `tools/build_decay_parent_map.py` carried a
`heavy_flavour_sign()` helper whose beauty convention was inverted relative to
`generation/producer/HeavyFlavourUtils.h` -- for beauty MESONS and for beauty
BARYONS alike. Its docstring described the opposite rule to the one it needed.

WHY NOTHING CAUGHT IT. The builder's I2 invariant compares a parent's sign to
...
THE AUTHORITY. `q_c = n_c - n_cbar`, `q_b = n_b - n_bbar`, decoded from PDG
digits by `DecodeHeavyContent`. Baryon codes are all-quark, so a positive beauty
...
Checks 1 and 2 fail against the pre-fix helper: it returned +1 for B+ (where
production gives -1) and -1 for Lambda_b0 (where production gives +1).
"""
import json
import sys
...
WORKED_EXAMPLES = {
    521: (5, -1),    # B+   = u bbar  -> q_b = -1   (the review's example)
    -521: (5, +1),   # B-
    511: (5, -1),    # B0   = d bbar
    -511: (5, +1),   # B0bar
    5122: (5, +1),   # Lambda_b0 = udb -> q_b = +1  (the review's example)
