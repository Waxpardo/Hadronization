#!/usr/bin/env python3
"""Require campaign-level A2 restoration while accepting zero-restoration jobs.

MONASH restores about six candidates per million events.
A 100,000-event job therefore restores 0.62 on average, making zero the modal outcome.
A per-job gate discarded 49 of 100 MONASH jobs and biased the comparison.
...
Case 1 prevents the other refusal checks from recreating the selection bias.
"""
import subprocess
import sys
import tempfile
