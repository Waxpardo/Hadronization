...
**`bins_to_ignore` is the specific defect, and it has already happened once.**
...
stack complains -- the B6 session found it only by looking at the rendering
(`docs/history/PLOTTING_B6_AND_V3_RUN_SESSION_20260813k.md`). This test asserts
the list is empty in every canvas of the generated configuration, so the same
copy cannot be made silently a second time.
"""
import json
import subprocess
import sys
