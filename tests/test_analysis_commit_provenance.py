...
def make_tree(tmp: Path, git: bool) -> Path:
    ...
    (base / "analysis/status_analysis_THnSparse_qq.C").write_text(
        'constexpr const char* kAnalysisSchema = "fixture_pair_schema_v1";\n'
    )
    (base / "config/pair_file_object_contract_v1.json").write_text(
        '{"schema_version_tags":{"v1":"fixture_pair_schema_v1"}}\n'
    )
