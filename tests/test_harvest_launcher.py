#!/usr/bin/env python3
"""Exercise every harvest-launcher refusal and its pipeline lock.

Each refusal runs once normally and once against a mutation that disables it.
"""
...
def main() -> int:  # noqa: C901
    ...
    try:
        ...
        # ---- 8. Refuse a lock from another host because its PID cannot be checked.
        box = _sandbox(stack)
        lock = box["workspace"] / "pipeline.lock"
        lock.parent.mkdir(parents=True, exist_ok=True)
        lock.write_text(
