...
// The associate/trigger acceptance, and the direct-primary status window.
//
// These four numbers were literals here while the SAME numbers also existed as
// named constants further down this header. Two spellings of one selection is
// what E10 was: a caption took 4.0 from the neighbouring context, and nothing
// could notice, because nothing tied the label to the predicate. A figure
...
inline constexpr int kDirectPrimaryStatusMin = 81;
inline constexpr int kDirectPrimaryStatusMax = 89;
...
inline bool IsDirectPrimaryStatus(int status) {
  ...
  return status > 0 && absolute >= kDirectPrimaryStatusMin &&
         absolute <= kDirectPrimaryStatusMax;
}
