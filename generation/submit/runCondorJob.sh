...
#
# CARD_VARIANT is new (2026-08-17, the systematics program). NONE means the
# nominal tune card, which is every campaign before this one; a token names a
# generated single-setting variation under generation/cards/systematics/. It
# does not weaken any guard: the card is still pinned by
# EFFECTIVE_CARD_SHA256, and the variant only decides which file that sha is
# checked against. It is placed before CLUSTER_ID/PROCESS_ID so those stay
# trailing, as they were.
#
# Otherwise the argument list is unchanged from the manifest-era worker, so
# ClassAds keep working. What is gone is the campaign-manifest
# authorisation chain: there is no sealed manifest to consult, no attempt claim
# to register, and no receipt cross-binding. What is kept is everything that
# can actually make the output wrong:
#
#   * the checkout must be at the commit the submit file recorded, and clean;
#   * the producer binary must hash to the value the submit file recorded;
#   * the materialised card must hash to the value the submit file recorded,
#     so a job cannot silently run a configuration nobody queued;
#   * output is validated before it is promoted, and never overwrites.
#
# Output location: $HF_PRODUCTION_ROOT if set, else <checkout>/Production.
# Keeping data off the checkout is what makes a second machine usable, so set
# it in config/dependencies.local.conf on any machine with real storage.
...
# This script lives at <checkout>/generation/submit/. It sat at the repository
# ROOT until the 2026-08-12 restructure moved it two levels down (RENAMES.md
# 1.1) WITHOUT updating this derivation, which was `${script_dir%/}` -- correct
# only while dirname $0 was the checkout. It therefore resolved to
# <checkout>/generation/submit, and every job exited 3 at the
# required-component check below, looking for
# generation/submit/generation/producer/...
#
# It went unnoticed for five days because the Nikhef checkout is still at the
# pre-restructure commit 43e35be8: NO production job has ever been launched
# from the restructured layout. The first one would have been a systematics
# variation.
#
# The assertion is the point, not the arithmetic: if this file is ever moved
# again, it must fail loudly here rather than resolve to a plausible wrong
# tree. HADRONIZATION_BASE cannot be supplied from outside -- it is on the
# forbidden-inherited list above -- so this derivation is the only source.
