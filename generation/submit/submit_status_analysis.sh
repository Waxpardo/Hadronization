...
usage() {
  cat <<'USAGE'
Usage:
  ./generation/submit/submit_status_analysis.sh FREEZE_DIR PRODUCTION_ROOT ANALYSIS_ROOT --dry-run
  ./generation/submit/submit_status_analysis.sh FREEZE_DIR PRODUCTION_ROOT ANALYSIS_ROOT --submit
...
python3 "${project_base}/tools/validate_analysis_outputs.py" \
  ...
  --allow-missing
# Log directories follow the tunes actually present in the manifest, so a
# configuration beyond the published three has somewhere to write.
while IFS= read -r manifest_tune; do
  mkdir -p "${analysis_root}/condor_logs/${manifest_tune}"
done < <(python3 -c '
import json, pathlib, sys
seen = []
for line in pathlib.Path(sys.argv[1]).read_text().splitlines():
    if line.strip():
        tune = json.loads(line)["tune"]
        if tune not in seen:
            seen.append(tune)
print("\n".join(seen))
