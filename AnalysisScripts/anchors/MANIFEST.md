...
| `f4_probe/` | historical defective map, corrected v1.1 map, and v2 map | **complete and verified** — see §1 |
...
| gap | status |
|---|---|
| **the merge log and run manifest** | the merge is **still running**; commit at completion. The private merge-band validation record is the timing authority because **the log carries no timestamps** — every timing there is reconstructed from filesystem mtimes and cannot be re-derived once scratch is cleared |
