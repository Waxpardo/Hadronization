...
Finished 2026-08-16 11:37:27 CEST after 13 h 29 m:
...
| | |
|---|---|
| total entries | **46,678,201** |
| per event (100 M events) | **0.4668** — plausible against the MONASH-calibrated ~0.54 |
| kCentralGround | **54.1697 %** |
| kExcludedVector | **39.9976 %** |
| kExcludedExcited | **5.7745 %** |
| kMultiplyHeavy | **0.0583 %** |
...
| I2, block vs central, robust MAD null | **1 flag** in 10 comparisons — see below |
...
## The I2 flag — recorded here because an anchor must not look cleaner than it is

One bin flags at |z| > 4: **Σ*_c⁺** (`kExcludedExcited`, 308,901 entries) reads
2.3 % low in block 2 at z = −4.28. It is **isolated in both directions** — its
conjugate in the same block is +1.16, block 2's total sits mid-pack among the
ten, and the next largest pull anywhere in that block is +2.54. Setting the
kMultiplyHeavy subpopulation aside, this is 1 flag in ~2,960 comparisons across
the three tunes against ~0.19 expected.
...
| sources | `hadronization_merged/complete_root_HF_RUN3_V1_CLOSEPACKING`, `…/combined_root_subSamples_CLOSEPACKING/combined_root_{1..10}` |
...
| remote outputs | `/data/alice/ipardoza/tune_runs_three/CLOSEPACKING/` |
...
extraction/decompose_with_block_sems.py \
  AnalysisScripts/anchors/merged_closepacking_dedup --tune CLOSEPACKING
```
