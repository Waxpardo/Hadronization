...
## Recompute the class labels

```bash
python3 tools/apply_class_labels.py --check
```

The shared class-label utility computes the fraction of the MONASH MB sample
**strictly below** each boundary. The cross-tune translation table in
`PRODUCTION_SHAPE_DECISION.md` remains the numerical record: all eleven entries
agree within 0.01 percentage points and the maximum residual is 2.91 percentage
points.
...
the private scratch-reconciliation record, Section 4.3.
...
**The private branch-state record's "WRITTEN — UNRUN — AVAILABLE" table therefore lists this macro
