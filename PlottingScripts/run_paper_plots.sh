...
require_file_for_provenance() {
  if [ ! -f "$1" ]; then
    echo "ERROR: final-plot provenance tool is absent: $1" >&2
    exit 1
  fi
}
require_file_for_provenance "${plot_provenance_tool}"
...
run_boundary_freeze() {
  ...
  root -l -b <<ROOTCMDS
.L PlottingScripts/improvedPlotting_THnSparse.C+
int __boundary_freeze_result =
  freezeMultiplicityBoundaries_THnSparse("${config}");
if (__boundary_freeze_result != 0) {
  gSystem->Exit(__boundary_freeze_result);
}
.q
ROOTCMDS
}
...
for target in "${expanded_targets[@]}"; do
  ...
  case "${target}" in
    ...
    final-multiplicity|final-yields)
      ...
      provenance_output_roots+=("PlottingScripts/FinalAnalysis/Plots")
      ;;
  esac
