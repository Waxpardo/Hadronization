...
# The analysis schemas this tool will accept, DERIVED from the pair-object
# contract rather than restated here, so the two cannot drift. The contract is
# the single place that says which schemas exist; a tool that kept its own copy
# is exactly how a contract change reaches one consumer and not its siblings.
ACCEPTED_ANALYSIS_SCHEMAS = frozenset(
    json.loads(
...
def validate_boundary_receipt(
    ...
    if (
        ...
        != sha256(
            checkout / "config/multiplicity_percentile_classes_v2.json"
        )
...
def _validate_pair_file_contract(
    ...
    # analysis_schema is deliberately NOT in exact_strings. It is an axis, not
    # a constant: the pair-object contract declares which schemas exist -- v2
    # carries six closure-checked content objects, v3 adds
    # hFlavourClosureSpecies -- and a file is judged against the one it
    # declares. An exact match against a single configured string would reject
    # a correct v3 file that the object contract accepts, which is the last
    # instance of the one-consumer blindness the v2-pin sweep removed
    # elsewhere. It is checked for MEMBERSHIP below, fail-closed.
    #
    # analysis_implementation and analysis_version stay exact: the producer did
    # not move them for the species axis, only the schema.
    exact_strings = {
    ...
    # Membership, not equality, and fail-closed on a tag the contract does not
    # declare. Derived from the contract rather than from this tool's config so
    # the two cannot drift.
    declared_schema = _root_string(root_file, "analysis_schema")
    if declared_schema not in ACCEPTED_ANALYSIS_SCHEMAS:
        raise ValueError(
            f"{path}: analysis_schema={declared_schema!r} is not declared by "
            f"config/pair_file_object_contract_v1.json "
            f"(known: {sorted(ACCEPTED_ANALYSIS_SCHEMAS)}); refusing to guess "
            "which object contract applies"
        )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "PASS multiplicity_boundary_receipt_v2.json generated by the "
