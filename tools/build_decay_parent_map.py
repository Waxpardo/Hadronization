...
Usage:
  tools/build_decay_parent_map.py PROBE_OUT --ordinals species_ordinals_v2.json \\
      --out decay_parent_map_v1.json
"""
...
def main() -> int:
    ...
    # This must run BEFORE I2. I2 compares parent and daughter using
    # heavy_flavour_sign on both sides, so an inverted helper passes it by
    # common inversion -- which is how the beauty convention stayed backwards
    # through every previous build (review finding A11). Pinning the helper to
    # the production values first is what makes I2's verdict mean anything.
    convention_rows = assert_production_convention(args.ordinals)

    # ---- I2 HEAVY-QUARK SIGN on EVERY row ----------------------------------
    # The check that caught the bug, now applied to all rows rather than only
    # to split ones. A parent may change heavy FLAVOUR (b -> c is a real weak
    # decay); what it may never do is keep the flavour and flip the sign.
    for s in species:
