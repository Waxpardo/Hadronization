...
def render_card(
    ...
    # The three publication cards are digest-pinned and keep their old
    # provenance path. Generated variation cards are resolvers, so their
    # comment links follow moved reports without editing the nominal bytes.
    for old, new in MOVED_REPORT_PATHS.items():
        nominal_text = nominal_text.replace(old, new)
