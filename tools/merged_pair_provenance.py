...
def optional_sha256(path: Path) -> str | None:
    ...
    Mirrors tools/validate_analysis_outputs.py:72-80, which already applies
    this to the same file. canonical_raw_validation_receipt.json is recorded
    provenance, not an input to any check: the gate layer wrote it, only
    freeze_seal.json survived, and build_canonical_manifest.py has no producer
    for it. This tool was deleted before 6f270b1 landed, so it never received
    that treatment and hard-failed the merge on a missing gate-era file.

    freeze_seal.json deliberately keeps the plain sha256(): it IS written, by
    build_canonical_manifest.py:269, so its absence is a real fault.
    """
