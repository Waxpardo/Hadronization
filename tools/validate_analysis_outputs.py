...
def canonical_jobs_per_tune(rows: list[dict]) -> int:
    ...
    if (
        ...
        raise ValueError(
            "canonical analysis validation requires equal N>=100, N%10=0 "
            f"tune exposure; counts={tune_counts} rows={len(rows)}"
        )
    expected = [
        (tune, slot)
        for tune in TUNES
...
def main() -> int:
    ...
    subprocess.run(
        [
            "python3",
            str(checkout / "tools/canonical_manifest.py"),
            "validate",
            str(freeze_dir),
        ],
        check=True,
    ...
    for row in rows:
        ...
        validate_analysis_raw_receipt(
            ...
            row["logical_id"],
            expected_log=(
                production_root / row["raw_validation_log_path"]
            ),
            expected_log_sha256=row["raw_validation_log_sha256"],
        )
    ...
    report = {
        ...
        "canonical_manifest_sha256": manifest_sha,
        "canonical_freeze_seal_sha256": sha256(freeze_dir / "freeze_seal.json"),
        "canonical_validation_receipt_sha256": sha256(
