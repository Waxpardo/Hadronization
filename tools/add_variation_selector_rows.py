...
def build_row(campaign: str, template: dict, prereg_sha: str, base: Path) -> dict:
    ...
    return {
        ...
        "subsample_base":
            f"{base_string}/hadronization_merged/SUBSAMPLES_{campaign}/combined_root_subSamples",
        "analyzed_data_base": f"{base_string}/hadronization_merged",
        "production_root": f"{base_string}/hadronization_production/{campaign}",
        "analysis_root": f"{base_string}/hadronization_analysis/{campaign}",
        "raw_base": f"{base_string}/hadronization_production/{campaign}/raw",
        "canonical_manifest":
            f"{base_string}/systematics_harvest/manifests/{campaign}/canonical_manifest.jsonl",
...
def main() -> int:
    ...
    ap.add_argument("--base", type=Path, required=True,
                    help="external campaign root; no site-specific default")
    ...
    for campaign in CAMPAIGNS:
        ...
        if key in combined["datasets"]:
            ...
            continue
        combined["datasets"][key] = build_row(
            campaign, template, prereg_sha, args.base.resolve())
        written.append(key)
