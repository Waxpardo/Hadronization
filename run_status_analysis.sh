...
raw_receipt_validator="${project_base}/tools/validate_analysis_raw_receipt.py"
expected_campaign=""
expected_tune=""
expected_logical_id=""
expected_raw_sha256=""
...
for required in "${input_file}" "${validator}" "${macro}" \
  "${raw_receipt_validator}"; do
  if [[ ! -e "${required}" ]]; then
...
if [[ -n "${raw_validation_receipt}" ]]; then
  ...
  python3 "${raw_receipt_validator}" \
    "${raw_validation_receipt}" "${input_file}" "${input_sha256}" \
    "${expected_raw_validation_receipt_sha256}" \
...
verify_analysis_checkout() {
  ...
  if [[ "${current_commit}" != "${analysis_commit}" ]] ||
     [[ "${current_macro_sha256}" != "${analysis_macro_sha256}" ]] ||
...
validate_existing_job_metadata() {
  ...
  python3 - "${metadata_path}" "${input_file}" "${input_sha256}" \
    ...
    "${expected_raw_validation_receipt_sha256}" <<'PY'
import json
import sys
