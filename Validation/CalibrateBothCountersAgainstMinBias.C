...
// S4: the |eta| < 4 cross-check counter, NCH_PRIMARY_CHARGED_ETA40_V1.
constexpr std::size_t kWideCounter = 4;
...
int CalibrateBothCountersAgainstMinBias(int events = 20000,
  ...
  // S4: the same fail-closed check on the wide slot. A reordered ladder must
  // not silently label one counter with another's name.
  const Counter& wide = counters[kWideCounter];
  if (wide.etaMax != 4.0 || wide.ptMin != 0.15 || !wide.excludeHeavy) {
    std::printf(
        "ERROR: counter[%zu] is not NCH_PRIMARY_CHARGED_ETA40_V1 "
        "(got etaMax=%.2f ptMin=%.2f excludeHeavy=%d).\n",
        kWideCounter, wide.etaMax, wide.ptMin, wide.excludeHeavy ? 1 : 0);
    return 7;  // 6 is already the tune-card read failure
  }
  ...
  // S4: the wide-counter distribution, same binning so the two are directly
  // comparable bin for bin.
  const std::string wideName = std::string("hNchWide_") +
                               (hardSample ? "hard" : "mb") +
                               (std::string(tuneLabel).empty()
                                    ? ""
                                    : std::string("_") + tuneLabel);
  TH1D nchWide(wideName.c_str(),
               "NCH_PRIMARY_CHARGED_ETA40_V1;N_{ch};events",
               kNchBins, kNchLow, kNchHigh);
  nchWide.SetDirectory(nullptr);
  nchWide.Sumw2();
  ...
  while (generated < events) {
    ...
    long long wideNch = 0;      // S4: per-EVENT, on the |eta| < 4 counter
    for (int i = 0; i < pythia.event.size(); ++i) {
      const Pythia8::Particle& p = pythia.event[i];
      ...
      for (std::size_t ci = 0; ci < counters.size(); ++ci) {
        ...
        if (p.pT() > c.ptMin && std::abs(p.eta()) <= c.etaMax) {
          ...
          if (ci == kWideCounter) ++wideNch;
        }
      }
    }
    nch.Fill(static_cast<double>(analysisNch));
    nchWide.Fill(static_cast<double>(wideNch));
  }
  ...
  if (writeNch) {
    ...
    const double wideOverflow = nchWide.GetBinContent(nchWide.GetNbinsX() + 1);
    if (wideOverflow > 0.0) {
      std::printf(
          "WARNING: %.0f events above N_ch = %.0f went to WIDE overflow.\n",
          wideOverflow, kNchHigh);
    }
    nch.Write();
    nchWide.Write();
    out.Close();
    std::printf(
        "NCH_WIDE_DISTRIBUTION_WRITTEN file=%s hist=%s entries=%.0f "
        "mean=%.4f overflow=%.0f sample=%s tune=%s\n",
        nchOutputPath, wideName.c_str(), nchWide.GetEntries(),
        nchWide.GetMean(), wideOverflow, hardSample ? "hard" : "mb",
        tuneLabel);
    std::printf(
