...
#
# additive_metadata_closure_checks is 3600: 12 checks for each of 300 pair files.
#
...
#
# The two manual checks cover input_file_count and source_input_events.
# Their expected relation depends on the block mode, which the contract does not encode.
...
if (( status != 0 )) ||
   ...
   grep -qE 'PAIR_BLOCK_CLOSURE_ERROR|segmentation violation|Break +segmentation|cling JIT session error' \
     "${log_file}"; then
  # Keep a failed validator's log for diagnosis; remove only successful logs.
