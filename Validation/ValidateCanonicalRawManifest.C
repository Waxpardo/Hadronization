...
constexpr int kFirstStageSlots = 100;
...
int ValidateCanonicalRawManifest(const char* manifestPath,
  ...
  while (std::getline(stream, line)) {
    ...
    const std::string attemptStartClaim =
        row.value("attempt_start_claim_path", "");
    const std::string attemptStartClaimSha =
        row.value("attempt_start_claim_sha256", "");
    ...
    if ((!firstStage && !superseding && !extension) ||
        ...
        slot < 0 ||
        (firstStage && slot >= kFirstStageSlots) ||
        ...
        row.value("raw_schema", "") != "hf_primary_ground_raw_v5" ||
        ...
        attemptStartClaim.empty() || !IsLowerHex(attemptStartClaimSha, 64) ||
  ...
  for (const auto& tune : kTunes) {
    ...
    const int minimumJobs =
        manifestSchema == kExtensionCanonicalSchema ? kBlocks
                                                    : kFirstStageSlots;
    if (tuneJobs != jobsPerTune || tuneJobs < minimumJobs ||
        tuneJobs % kBlocks != 0) {
  ...
  if (manifestSchema == kCanonicalSchema &&
      jobsPerTune != kFirstStageSlots) {
    std::cerr << "CANONICAL_RAW_ERROR v2 first-stage exposure differs\n";
    ++errors;
  }
