...
HADRONIZATION_CANONICAL_SEALING=1 \
python3 "${project_base}/tools/canonical_manifest.py" validate \
  "${freeze_dir}" --allow-unsealed
shape="$(
python3 -c '
...
python3 "${project_base}/tools/canonical_manifest.py" seal \
  "${freeze_dir}" "${production_root}" "${validation_log}"
