...
# The manifest's own structural validation used to happen here via the
# canonical-manifest sealing tool. What that tool checked beyond the block
# below was authorisation bookkeeping; the checks that matter -- every raw file
# present, every checksum matching, seeds unique -- are done inline.
shape="$(
python3 -c '
