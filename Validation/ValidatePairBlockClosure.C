...
// expectedSchema is the schema the CAMPAIGN requires, supplied by the caller.
// It is REQUIRED (review finding A4): this macro used to adopt whatever schema
// the first central file declared and then check everything against that, so a
// complete, internally consistent v2 directory passed at 1800/600 -- the exact
// state README says must be treated as failure. Expectations taken from the
// thing under test cannot fail it. An empty expectedSchema is refused rather
// than defaulted, because a default here would silently restore the old
// behaviour for every caller that had not been updated.
