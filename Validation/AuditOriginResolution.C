...
ValidationReceiptEvidence VerifyValidationReceipt(
  ...
  // Three receipt generations, and the version string alone says which
  // guarantees a receipt carries -- do not probe for fields to decide this.
  //
  //   v1  gate era.   result / validator_exit_status.  Has output_bytes and
  //                   validator_dependency_sha256.
  //   v2  4d9dfa7.    state / validator_status.  Has NEITHER: the writer was
  //                   rewritten smaller and these two readers were not
  //                   revisited. A WEAKER contract -- the input-size and
  //                   validator-dependency bindings cannot be checked at all.
  //                   Accepted so the HF_PT2 campaign stays auditable.
  //   v3  current.    state / validator_status, and both fields restored.
  //
  // A v2 receipt is therefore accepted with a genuinely weaker check. That is
  // a property of the receipt, not a relaxation of this validator: there is no
  // stored evidence to compare against. New production writes v3.
  ...
  if (isV2) {
    std::cerr << "RAW_RECEIPT_WEAK_BINDING schema=" << receiptSchema
              << " path=" << path
              << " (no output_bytes / validator_dependency_sha256; "
                 "input-size and validator-dependency bindings unchecked)\n";
  }
  ...
  // v1 and v3 carry the full block; v2 carries five of these fields at top
  // level and nothing else. Compare whatever the receipt actually recorded,
  // decided by version rather than by probing.
  if (isV2) {
    for (const char* key :
         {"campaign", "tune", "logical_id", "attempt", "repository_commit"}) {
      if (!receipt.contains(key) || receipt[key] != expected[key]) {
        throw std::runtime_error(
            std::string("raw-validation receipt provenance differs at ") + key);
      }
    }
  } else {
    ...
    // The attempt-start claim digest is required of v1 only. Claims were
    // removed with the gate layer, so a v3 receipt cannot carry one and
    // demanding it would reject every receipt this pipeline will ever write.
    if (isV1) {
