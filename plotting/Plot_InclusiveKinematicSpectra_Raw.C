...
// ---------------------------------------------------------------------------
// The canonical-freeze contract. THE CONTRACT DESCRIBES THE SEAL, NEVER THE
// REVERSE.
//
// WHAT THIS REPLACES, AND WHY IT HAD TO CHANGE. The previous contract required
// a five-artifact freeze and hardcoded one campaign's decomposition:
// `jobs_per_tune == 100` and `requested_successes == 1000000`. Those numbers
// describe HF_100M_primaryGround_ccbb_v1. HF_RUN3_V1 is 1000 jobs x 100000
// events -- the SAME 100M per tune, differently divided -- so a correct,
// sealed, fully validated freeze was refused for having the wrong shape. A
// contract that encodes one campaign's arithmetic is a contract that expires.
//
// Two of the five artifacts it demanded, `freeze_summary.json` and
// `canonical_raw_validation_receipt.json`, are written NOWHERE in this
// repository outside test fixtures. `Validation/validate_canonical_manifest.sh`
// records why: the sealing tool that used to emit them was removed because
// "what that tool checked beyond the block below was authorisation
// bookkeeping", and the checks that matter -- every raw file present, every
// checksum matching, seeds unique -- were kept inline. This contract follows
// that same ruling rather than resurrecting the bookkeeping layer.
//
// THE ARTIFACT SET, narrowed to what a sealed freeze actually contains:
//
...
//
// The per-job validation evidence is the substantive kind: 3000 rows each
// naming their own validation receipt and its digest. A campaign-level PASS
// would be an aggregate of those, and this session declined to synthesise one:
// a receipt asserting a validation event that never ran is a lie regardless of
// how it is framed.
//
// THE SHAPE IS DERIVED, NEVER ASSUMED. Tunes, jobs per tune, events per job and
// the block width all come out of the manifest, and the seal is then required
// to agree with what the manifest says about itself. Internal consistency is
// asserted rather than trusted: rows == tunes x jobs_per_tune, events uniform
// across every row, per-tune exposure equal, blocks equal, slots contiguous,
// seeds and paths unique.
//
// STILL FAIL-CLOSED. Missing manifest or seal, a seal that disagrees with its
// manifest, a tampered digest, non-uniform events, unequal tunes, a short block
// or a duplicated slot all refuse. The gate got more general, not weaker.
// ---------------------------------------------------------------------------
...
// The multiplicity figure's own selection, taken from the constants the
// PRODUCER counts with -- not written out by hand.
//
// The hand-written version said "|eta| <= 4". That is the ASSOCIATE acceptance
// (kMultiplicityEtaWide, and IsCentralKinematic's limit for the pT/eta/phi
// spectra). The quantity actually plotted here is hMULTIPLICITY, which the
// producer fills from multiplicityCentral -- counted at
// kMultiplicityEtaCentral = 1.0. So the figure stated an acceptance four times
// wider in eta than the number it drew.
//
// Deriving both lines from the same constants the counter uses makes the label
// unable to disagree with the histogram: change the cut and the caption moves
// with it.
...
// same rule, and the same source files, as
// plotting/paper/make_paper_figures.py::boundary_percentiles(). Recomputed
// rather than transcribed so a label cannot drift away from the sample it
// claims to describe.
// ---------------------------------------------------------------------------
...
struct CommonBoundaryMarker {
  ...
  // docs/plotting_validation/hf_run3_v1_threetune_20260816/
...
std::vector<CommonBoundaryMarker> CommonBoundaryMarkers(const std::string& base)
{
  ...
  // The labels are MONASH's by ruling, whatever tune is drawn.
                               ...
                               bool normalizeShape)
{
  ...
  TPad* inset = new TPad(("pMonashPercentiles_" + outputStem).c_str(),
                         ...
                         // MEASURED, 2026-08-18. The old frame ran to
                         // NDC x 0.58 = N_ch 89.8 and its top edge sat at
                         // y = 2.86e-5, but all three spectra fall below that
                         // level by N_ch 71-75, so every one of them entered the
                         // frame and -- once the fill went transparent -- was
                         // visibly drawn across it. This frame reaches N_ch 72.6
                         // with a top edge at y = 9.4e-6, which the earliest
                         // crossing (N_ch 77) clears by 4.4 units.
                         0.18, 0.07, 0.50, 0.38);
  // OWNER MANDATE, 2026-08-18: the inset background is TRANSPARENT.
  //
  // It was opaque white (fill style 1001), which hid whatever spectra ran
  // beneath it. Hiding a collision is not the same as not having one -- the
  // curves were always there, and an opaque panel simply meant the reader could
  // not see what had been covered up. Fill style 0 paints no background, so the
  // pad shows the spectra through it and any genuine overlap becomes visible
  // and can be judged by eye.
                 ...
                 bool logY)
{
  ...
  // HEADROOM, owner ruling 2026-08-18. The caption sits ABOVE the frame, so the
  // top margin carries it. 0.28 puts the frame top at 0.720 against a caption
  // bottom edge of 0.738. See kCaptionFirstBaseline for the preview that set it.
