...
// This list was hand-maintained and had drifted from the pair-file
// contract: it was missing both hCorrelationsByOrigin and hFlavourClosure,
// so the plotting-input validator never required two of the objects every
// pair file carries. It now derives from the generated contract, the same
// source the pair-directory allowlist and the ten-block closure use.
//
// It is also no longer a constant. The object set is a function of the schema
// a file declares, so kCommonObjectNames holds only the OLDEST version's set --
// the objects every pair file carries whatever version it is -- and is used
// only where no file is in hand, i.e. the report header. Every actual check
// resolves the version from the file itself through ObjectNamesFor.
const std::set<std::string> kCommonObjectNames =
    Hadronization::ClosureCheckedContentObjects(
        Hadronization::PairSchemaVersion::kV2);

// Resolves a file's declared schema, FAILING CLOSED. A file that names a
// schema this contract does not know is not validated against a guessed one.
Hadronization::PairSchemaVersion PairSchemaVersionOf(TFile& file,
                                                    const std::string& path) {
    auto* declared = dynamic_cast<TObjString*>(file.Get("analysis_schema"));
    if (!declared) {
        throw std::runtime_error("No analysis_schema in " + path);
    }
    const std::string tag = declared->GetString().Data();
    Hadronization::PairSchemaVersion version;
    if (!Hadronization::ParsePairSchemaVersion(tag, version)) {
        throw std::runtime_error("Unknown analysis_schema '" + tag + "' in " +
                                 path +
                                 "; refusing to guess which object contract "
                                 "applies");
    }
    return version;
}

std::set<std::string> ObjectNamesFor(TFile& file, const std::string& path) {
    return Hadronization::ClosureCheckedContentObjects(
        PairSchemaVersionOf(file, path));
}
...
std::string ProjectBase()
{
    ...
    if (configured && Exists(Join({configured, "plotting"}))) return configured;
...
// The campaign shape used to live in this file as literals: 100 jobs per tune
// and 1,000,000 successful events per job. No campaign this repository is
// configured to produce has ever had that shape -- Makefile:25-26 renders full
// production as 1000 jobs x 100,000 events -- so the literals rejected every
// real dataset, HF_PT2 and HF_RUN3_V1 alike. The shape is now read from the
// campaign's own freeze seal, which tools/build_canonical_manifest.py:306-320
// writes next to the manifest and which every manifest row already pins
// through source_freeze_seal_sha256. The seal is an input, so it is checked
// for internal consistency rather than trusted.
struct CanonicalCampaignShape {
    std::string campaign;
    std::string sealPath;
...
std::string FreezeSealBesideManifest(const std::string& manifestPath)
{
    const std::string absolute = AbsoluteFromBase(manifestPath);
    const std::size_t slash = absolute.find_last_of('/');
    if (slash == std::string::npos) {
        throw std::runtime_error(
            "Canonical manifest path has no directory component: " +
            absolute);
    }
    return absolute.substr(0, slash) + "/freeze_seal.json";
}

std::string FreezeSealFromEnvironment()
{
    const char* manifest =
        std::getenv("HADRONIZATION_CANONICAL_MANIFEST");
    if (!manifest || std::string(manifest).empty()) {
        throw std::runtime_error(
            "Canonical validation requires HADRONIZATION_CANONICAL_MANIFEST "
            "so the campaign freeze seal can be located; the dataset "
            "selector exports it for canonical and canonical_candidate rows");
    }
    return FreezeSealBesideManifest(manifest);
}

CanonicalCampaignShape LoadCanonicalCampaignShape(
    const std::string& sealPath)
{
    CanonicalCampaignShape shape;
    shape.sealPath = sealPath;
    std::ifstream input(sealPath);
    if (!input) {
        throw std::runtime_error(
            "Missing canonical freeze seal: " + sealPath);
    }
    json seal;
    input >> seal;
    const std::string sealSchema = seal.value("schema", std::string());
    if (sealSchema != "hf_canonical_freeze_seal_v2" &&
        sealSchema != "hf_superseding_canonical_freeze_seal_v3") {
        throw std::runtime_error(
            "Unsupported canonical freeze-seal schema in " + sealPath);
    }
    for (const char* key : {"campaign", "jobs_per_tune", "blocks", "rows",
                            "tunes", "total_requested_successes"}) {
        if (!seal.contains(key)) {
            throw std::runtime_error(
                "Canonical freeze seal is missing '" + std::string(key) +
                "': " + sealPath);
        }
    }
    shape.sealSha256 = Hadronization::Sha256FileHex(sealPath);
    shape.campaign = seal.at("campaign").get<std::string>();
    shape.jobsPerTune = seal.at("jobs_per_tune").get<int>();
    shape.blocks = seal.at("blocks").get<int>();
    shape.rows = seal.at("rows").get<int>();
    shape.tunes = static_cast<int>(seal.at("tunes").size());
    const long long totalEvents =
        seal.at("total_requested_successes").get<long long>();

    if (shape.campaign.empty() ||
        shape.blocks != 10 ||
        shape.tunes != static_cast<int>(kTunes.size()) ||
        shape.jobsPerTune < shape.blocks ||
        shape.jobsPerTune % shape.blocks != 0 ||
        shape.rows != shape.jobsPerTune * shape.tunes ||
        totalEvents <= 0 ||
        totalEvents % shape.rows != 0) {
        throw std::runtime_error(
            "Canonical freeze seal is not internally consistent: " +
            sealPath);
    }
    shape.eventsPerJob = totalEvents / shape.rows;
    return shape;
}
...
CanonicalManifestEvidence ReadCanonicalTuneManifest(
    ...
    int expectedBlock,
    int centralSlots,
    const CanonicalCampaignShape& shape)
{
    ...
    if (expectedBlock < 0) {
        // Both manifest generations must expose exactly the per-tune job
        // count the seal declares. For a superseding manifest that count is
        // the final union size, so this is an equality in both cases; what
        // distinguishes v3 is that the union draws on at least two sealed
        // source campaigns.
        const bool rowsMatchSeal = derivedRows == shape.jobsPerTune;
        const bool validFirstStage =
            manifestSchema == "hf_canonical_raw_manifest_v2" && rowsMatchSeal;
        const bool validSuperseding =
            manifestSchema ==
                "hf_superseding_canonical_raw_manifest_v3" &&
            rowsMatchSeal && sourceContracts.size() >= 2;
        if (!validFirstStage && !validSuperseding) {
            throw std::runtime_error(
                "Canonical central manifest does not expose the sealed "
                "per-tune job count N=" +
                std::to_string(shape.jobsPerTune) + " (found " +
                std::to_string(derivedRows) + "): " + manifestPath);
        }
        for (const auto& expectedTune : kTunes) {
        ...
        const bool slotsMatchSeal = centralSlots == shape.jobsPerTune;
        const bool validFirstStage =
        ...
        const bool validSuperseding =
            manifestSchema ==
                "hf_superseding_canonical_raw_manifest_v3" &&
            slotsMatchSeal && sourceContracts.size() >= 2;
        if ((!validFirstStage && !validSuperseding) ||
            derivedRows != centralSlots / shape.blocks) {
            throw std::runtime_error(
                "Canonical block exposure differs from central manifest: " +
                manifestPath);
        }
        for (const auto& expectedTune : kTunes) {
            for (int slot = expectedBlock; slot < centralSlots;
                 slot += shape.blocks) {
                if (!tuneSlots[expectedTune].count(slot)) {
                    throw std::runtime_error(
...
// Returns ownership: TFile::Get does not adopt THnSparse, and this runs
// over 300 central plus 3000 block files.
std::unique_ptr<TObject> CheckedObject(TFile& file, const std::string& objectName, const std::string& path)
{
    std::unique_ptr<TObject> object(file.Get(objectName.c_str()));
    const bool correct = objectName == "summed MULTIPLICITY"
        ? dynamic_cast<TH1D*>(object.get()) != nullptr
        : dynamic_cast<THnSparseD*>(object.get()) != nullptr;
    if (!correct) {
        throw std::runtime_error(
...
void CheckKeyType(TFile& file, const std::string& objectName, const std::string& path)
{
    ...
    // The expected ROOT class comes from the contract rather than from a
    // name special-case, so a new content object cannot be type-checked
    // against the wrong class by default.
    std::string expectedClass;
    for (const auto& object : Hadronization::kPairObjects) {
        if (objectName == object.name) { expectedClass = object.rootClass; break; }
    }
    const bool correct = expectedClass == "THnSparse"
...
int Validate_THnSparse_Production(
    ...
    const char* reportPath =
        "plotting/validation/final_thnsparse_input_validation.json")
{
    ...
    json report = {
        ...
        {"required_objects_common_to_all_schemas", kCommonObjectNames},
        {"tunes", json::object()},
    ...
    // Loaded lazily: only the canonical path has a freeze seal, and the
    // legacy path must keep working without one.
    CanonicalCampaignShape canonicalShape;
    bool canonicalShapeLoaded = false;
    ...
    for (const auto& tune : kTunes) {
        ...
        if (canonicalMode) {
            if (!canonicalShapeLoaded) {
                canonicalShape =
                    LoadCanonicalCampaignShape(FreezeSealFromEnvironment());
                canonicalShapeLoaded = true;
                std::cout << "CANONICAL_CAMPAIGN_SHAPE campaign="
                          << canonicalShape.campaign
                          << " jobs_per_tune=" << canonicalShape.jobsPerTune
                          << " blocks=" << canonicalShape.blocks
                          << " events_per_job=" << canonicalShape.eventsPerJob
                          << " seal_sha256=" << canonicalShape.sealSha256
                          << std::endl;
            }
            const std::string manifestPath =
                Join({centralDir, "source_manifest.jsonl"});
            centralManifest = ReadCanonicalTuneManifest(
                manifestPath, tune, 0, -1, 0, canonicalShape);
            ...
            if (canonicalMode) {
                ...
                const CanonicalManifestEvidence sampleManifest =
                    ReadCanonicalTuneManifest(
                        manifestPath, tune,
                        centralManifest.rowsPerTune / canonicalShape.blocks,
                        sample - 1,
                        centralManifest.rowsPerTune, canonicalShape);
        ...
        for (const auto& fileName : centralNames[tune]) {
            ...
            for (const auto& objectName :
                 ObjectNamesFor(centralFile, centralPath)) {
            ...
            for (int sample = 1; sample <= nSubsamples; ++sample) {
                ...
                for (const auto& objectName :
                     ObjectNamesFor(sampleFile, samplePath)) {
        ...
        for (const auto& fileName : comparisonPairs) {
            ...
            const std::string centralPath = Join({centralDir, fileName});
            // Resolved once, before the per-object loop reopens the file.
            const std::set<std::string> comparisonObjects = [&] {
                TFile probe(centralPath.c_str(), "READ");
                if (probe.IsZombie()) {
                    throw std::runtime_error("Cannot open " + centralPath);
                }
                return ObjectNamesFor(probe, centralPath);
            }();
            for (const auto& objectName : comparisonObjects) {
                TFile centralFile(centralPath.c_str(), "READ");
                if (centralFile.IsZombie()) throw std::runtime_error("Cannot open " + centralPath);
                const std::unique_ptr<TObject> ownedCentral =
                    CheckedObject(centralFile, objectName, centralPath);
                TObject* centralObject = ownedCentral.get();
                ...
                for (int sample = 1; sample <= nSubsamples; ++sample) {
                    ...
                    TFile sampleFile(samplePath.c_str(), "READ");
                    const std::unique_ptr<TObject> ownedSample =
                        CheckedObject(sampleFile, objectName, samplePath);
                    TObject* sampleObject = ownedSample.get();
    ...
    int expectedFilesPerTune = 110)
{
    int errors = 0;
    try {
        // Both manifests are inside the freeze directory here, so the seal is
        // their sibling rather than coming from the dataset selector.
        const CanonicalCampaignShape shape = LoadCanonicalCampaignShape(
            FreezeSealBesideManifest(centralManifestPath));
        const CanonicalManifestEvidence central =
            ReadCanonicalTuneManifest(
                centralManifestPath, "MONASH",
                expectedFilesPerTune, -1, 0, shape);
        const CanonicalManifestEvidence block =
            ReadCanonicalTuneManifest(
                blockManifestPath, "MONASH",
                expectedFilesPerTune / 10, 0,
                expectedFilesPerTune, shape);
