...
`improvedPlotting_THnSparse.C`; `attic/plotting/improvedPlotting.C` and earlier
plotting macros remain legacy.
...
- `configuration_multiplicity_HF_RUN3_V1_MONASH_THnSparse_complete_root.json`:
  the v3 campaign, MONASH only — what the first v3 run used while CLOSEPACKING
  was still merging.
- `configuration_multiplicity_HF_RUN3_V1_THREETUNE_THnSparse_complete_root.json`:
  **the v3 campaign, all three tunes**, including the four
  `drawBalancingPlotsTUNERatios` canvases. **Generated, not hand-edited.**
- `make_hf_run3_v1_three_tune_config.py`: generates that configuration from the
  MONASH one. The three-tune file repeats one canvas block ten times with only
  the tune, the pad rectangle and the ratio denominator changing, and nothing in
  the ROOT stack checks that the ten agree about anything else. `--check`
  asserts the committed file is what the generator produces;
  `tests/test_three_tune_plot_config.py` runs that and additionally pins
  `bins_to_ignore == []` on every canvas.

  > **Why `bins_to_ignore` is pinned.** The v2 reduced configuration lists ten
  > of the eleven multiplicity classes there. A canvas built by copying it draws
  > **one** class, faithfully, and nothing in the stack complains — the B6
  > session found it only by looking at the rendering.

  **Publication polish, 2026-08-16.** The generated configuration also carries
  the presentation settings the polished canvas depends on: a `canvas_title` per
  canvas so **every panel names its tune**, the legend on **one** panel with
  `legend_columns: 4`, ratio panels on a **linear 0.6–2.5** axis chosen from the
  measured 0.6605–2.2837 spread, and a 0.10 top margin so ROOT's pad-top titles
  clear the frame. `docs/plotting_validation/hf_run3_v1_threetune_20260816/RUN_RECORD.md`
  appendix records each with its reason and proves no number moved.
...
`analysis/status_analysis_THnSparse_qq.C`, then merged from a sealed
canonical manifest.
...
python3 tools/final_plot_provenance.py verify \
  --checkout "$PWD" \
  --sidecar plotting/Plots/THnSparse/<plot>_PDF.pdf.provenance.json
```
...
```bash
./plotting/run_paper_plots.sh list
```
...
./plotting/run_paper_plots.sh validate-inputs
./plotting/run_paper_plots.sh audit-subsamples

VERBOSE=true ./plotting/run_paper_plots.sh smoke \
  2>&1 | tee logs/final_paper_plots_smoke.log

VERBOSE=true ./plotting/run_paper_plots.sh thnsparse \
  2>&1 | tee logs/final_paper_plots_thnsparse.log

./plotting/run_paper_plots.sh multiplicity-spectrum
./plotting/run_paper_plots.sh all
```
...
```bash
./plotting/validate_subsample_log.py \
  logs/final_paper_plots_thnsparse.log \
  --json-output \
plotting/validation/final_thnsparse_uncertainty_validation.json
```
...
```text
plotting/Plots/THnSparse/
plotting/Plots/THnSparseCompleteRoot/
plotting/Plots/MultiplicityDistribution/
...
- `attic/plotting/improvedPlotting.C`;
- `attic/plotting/B_Balancing_GeneralPlotting.C`;
- old pT JSON configs with personal absolute paths
  (`attic/plotting/configuration_{pT,pseudorapidity,rapidity}.json`);
- `docs/history/studies/Balancing_and_Sampling/` plot/error code.

`combinedCanvasPlots.C`, `PlottingWizard.C` and `ListHistos.C` were removed from
`HEAD` on 2026-08-17 — no consumer, no recipe, no recorded run. They remain
reachable in git history; the manifest is [`../docs/REMOVALS.md`](../docs/REMOVALS.md).
