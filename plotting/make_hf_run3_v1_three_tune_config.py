#!/usr/bin/env python3
"""Generate the three-tune HF_RUN3_V1 plotting configuration from the MONASH one.

WHY GENERATED RATHER THAN HAND-EDITED. The MONASH v3 configuration is ~600 lines
and the three-tune version repeats its canvas block ten times with only the tune,
the pad rectangle and the ratio denominator changing. Hand-copying that is how
one canvas ends up with a different y-range, or a stale `bins_to_ignore`, and
nothing in the stack would catch it -- the B6 session lost a figure exactly that
way and only found it by looking at the rendering.

WHAT IS CARRIED OVER UNCHANGED from the MONASH configuration, deliberately:
the v3 pair-input selection contract, the eleven common multiplicity classes and
their legend text, the dependency line styles, the data paths, and
`bins_to_ignore: []`.

  **`bins_to_ignore` stays empty and that is the whole point.** The v2 reduced
  configuration lists ten of the eleven classes there, so a canvas built from it
  draws ONE class and looks entirely plausible
  (`docs/history/PLOTTING_B6_AND_V3_RUN_SESSION_20260813k.md`). This generator
  asserts the list is empty rather than trusting the copy.
...
# x: beauty left, charm right. y: MONASH, JUNCTIONS, CLOSEPACKING going up,
# with the two ratio rows beneath. Copied from the v2 reduced configuration,
# which is the layout that has actually been rendered and read.
