...
struct canvasConfigs {
    ...
    // THE IN-FRAME ROW LABEL (ruling R46 item 4): the tune name alone, small,
    // no box, in the panel's top-left. Empty on a panel that does not vary by
    // tune, which is the combined-ratio row and G8's overlaid yield row.
...
// ONE HEADER OVER ONE COLUMN (R46 item 3). The rectangle is the column's
// own, copied from the configuration that placed the panels, so the header
// follows the column instead of repeating its arithmetic.
struct ColumnHeader {
    std::string text;
    Double_t xMin;
    Double_t xMax;
...
// ONE ENTRY OF THE CANVAS LEGEND (R46 item 5, refined by ruling L1). `kind`
// is `tune` or `class_fill`. A tune entry resolves its colour and marker from
// plotting/TunePlotStyle.h, never from the configuration, so the palette
// stays in the header (owner decision O3).
...
struct globalCanvasConfigs {
    ...
    std::vector<CanvasLegendEntry> canvasLegend;
...
struct CONFIGS {
    ...
    // THE INFORMATION BLOCK, ONE ELEMENT PER LINE (ruling R46 item 2). It was
    // a single sentence about axis coverage; it now also carries the
    // generator, system and beam energy that every panel title used to
    // repeat. A configuration may still give one string, which reads as a
    // one-line block, so the frozen base and the hand-maintained files are
    // unaffected.
    std::vector<std::string> axisDeclaration;
};
...
//             four template sites and to the correlation title, and ROOT then
//             draws no title pad.
//
// THE R46 SCHEME, IN THREE LINES (ruling R46, owner 2026-09-01; refined by
// L1, L2 and L3 the same day).
//   titles off      `DRAW_CANVAS_TITLES = False` in
//                   tools/make_variant_configs.py. Every panel used to carry
//                   `TUNE, TRIGGER trigger, pp #sqrt{s} = 13.6 TeV`, which
//                   states the beam energy six times on an eight-panel
//                   composite.
//   where each      generator + system + energy and the axis-coverage
//   class is drawn  sentence go to the INFORMATION BLOCK, top left
//                   (`DrawInformationBlock`); the trigger, or the flavour on
//                   the baryon/meson and closure canvases, goes to a COLUMN
//                   HEADER above the top row (`DrawColumnHeaders`); the tune
//                   goes to an in-frame ROW LABEL in each panel's top-left
//                   (`DrawPanelLabel`); the conventions the panels share --
//                   the three tunes, and the open/filled class fill on the
//                   extremes canvases -- go to ONE CANVAS LEGEND, top right
//                   and outside every pad (`DrawCanvasLegend`).
//   the caption     the observable and its definition, which row is which,
//   carries         the multiplicity scope in words, and the R43 disclosure
//                   that B_c- is absent from the extreme-class beauty
//                   columns. The list per figure is the Phase-A map in
...
// THE INFORMATION BLOCK, PARSED (ruling R46 item 2).
//
// ONE KEY, TWO SHAPES. A generated configuration writes an ARRAY of lines; a
// hand-maintained one and the frozen base write nothing, and any older file
// may still carry the single STRING the axis declaration used to be. All
// three are accepted, and a string reads as a one-line block, so nothing this
// generator does not write has to change to keep rendering.
//
// EMPTY LINES ARE DROPPED rather than drawn: a blank line in the stack would
// open a gap the reader reads as a separation between two blocks.
std::vector<std::string> ParseInformationBlock(const json& config)
{
    std::vector<std::string> lines;
    if (!config.contains("axis_declaration")) { return lines; }
    const json& declaration = config["axis_declaration"];
    if (declaration.is_string()) {
        const std::string one = declaration.get<std::string>();
        if (!one.empty()) { lines.push_back(one); }
        return lines;
    }
    if (declaration.is_array()) {
        for (const auto& line : declaration) {
            if (!line.is_string()) {
                throw std::runtime_error(
                    "axis_declaration must hold strings; the information "
...
//
// SETTING THE FONT ON THE LEGEND IS NOT ENOUGH. A `TLegendEntry` is built
// with font 0, meaning "inherit from the legend", and ROOT RESOLVES THAT AT
// THE FIRST PAINT and writes the resolved number into the entry for good. A
// legend that is painted once before this pass runs therefore keeps whatever
// font it had at that moment -- 42, `gStyle`'s default -- and no later
// `SetTextFont(43)` on the legend can reach the entries again.
//
// WHY THAT DESTROYS THE TEXT RATHER THAN MERELY RESIZING IT. Font 42 is
// precision 2, whose size is a FRACTION OF THE PAD HEIGHT; font 43 is
// precision 3, whose size is in PIXELS. The entries keep font 42 and take the
// legend's size, 33, so every entry is drawn 33 pad-heights tall and lands
// outside the canvas. The marker samples do not use the text size, so they
// still draw -- which is why the delivered G8 and G5/G7 show markers with
...
// would fix these six legends and leave the trap for the next caller; setting
// the entries here fixes every consumer of this function, whenever it runs.
// The entries are put back to size 0 so they inherit the legend's pixel size
// and one number stays in charge of the metric.
void ApplyPublicationLegendStyle(TLegend* legend, Double_t canvasWidthPx)
{
    if (!legend) return;
    const Int_t entryPixels =
        PublicationTextPixels(kPublicationLabelFraction, canvasWidthPx);
    legend->SetTextFont(43);
    legend->SetTextSize(entryPixels);
    TIter nextEntry(legend->GetListOfPrimitives());
    while (TObject* object = nextEntry()) {
        if (TLegendEntry* entry = dynamic_cast<TLegendEntry*>(object)) {
...
void ApplyPublicationTextStyleToPad(TPad* pad, Double_t canvasWidthPx)
{
    ...
    while (TObject* primitive = nextPrimitive()) {
        if (TPad* child = dynamic_cast<TPad*>(primitive)) {
            ...
            ApplyPublicationLegendStyle(legend, canvasWidthPx);
        } else if (TLatex* inFrame = dynamic_cast<TLatex*>(primitive)) {
            // THE IN-FRAME ROW LABEL TAKES THE METRIC TOO (R46 item 4). It is
            // drawn on the mini pad, where the final canvas width is not yet
            // known, so it is authored at a placeholder size and sized here
            // like every other text on the assembled figure. The three
            // canvas-level labels are drawn AFTER this pass and set their own
            // size, so they are not reached by this branch.
            inFrame->SetTextFont(43);
            inFrame->SetTextSize(
                PublicationTextPixels(kPublicationLabelFraction, canvasWidthPx));
        }
    }
...
// THE CORRELATION PANELS' Y RANGE (architect finding F9, measured 2026-09-01).
//
// THE RANGE WAS 1e-6 TO 1e-2 and the charm figure showed two empty decades
// below its data. Measured over every drawn point of the delivered macros
// (`DELIVERABLES_REVIEW_20260901/G2_G3`, bin content, both triggers):
//
//   CHARM   1.324e-04 .. 6.207e-03
//   BEAUTY  4.204e-06 .. 3.279e-03
//
// THE TWO FLAVOURS SHARE ONE RANGE, which is what F9 requires so that the
// charm and beauty figures can be read against each other. That constraint
// sets the floor: BEAUTY'S OWN DATA REACHES 4.2e-06, so the decades that look
// empty on the charm figure are occupied on the beauty one. The range can
// therefore tighten only from 1e-6 to 2e-6 -- 0.3 of a decade -- and the
// charm figure keeps most of its empty span. Removing that span would need a
// per-flavour range, which F9 excludes; the report states the measurement and
// leaves the choice with the owner rather than splitting the two figures
// unasked.
//
// The ceiling comes down from 1e-2 only as far as the higher flavour's
// maximum allows with headroom for its error bar. Four decade labels remain
// under ruling L3.
// THE CORRELATION CANVAS RESERVES THE SAME BAND THE COMPOSITES DO (R46).
//
// Its four pads used to tile the whole canvas, so it had nowhere to state the
// generator, the system, the beam energy or its column identification -- and
// the delivered G2 and G3 state none of them. The generator gives every
// composite a 0.10 band by stopping its pads at 0.90
// (`CANVAS_LABEL_BAND_TOP`); this canvas is built in code, so the same
// reservation is made here and the two stay in step by carrying the same
// number in both places, each with a comment naming the other.
//
// MEASURED, NOT ASSUMED. Replayed with the pads still reaching 1.00, the
// block had to go inside the first panel's frame and landed on the OS/SS
// legend, which the away-side peak had already pushed to the upper LEFT of
// that same panel. The band removes the competition instead of tuning two
// boxes against each other.
constexpr Double_t kCorrelationRowTop = 0.90;
constexpr Double_t kCorrelationRowSplit = 0.45;

constexpr Double_t kCorrelationYieldMin = 2e-6;
constexpr Double_t kCorrelationYieldMax = 1e-2;

// THE IN-FRAME ROW LABEL (ruling R46 item 4).
//
// ONE WORD REPLACES A FORTY-CHARACTER TITLE. Every panel of a composite used
// to carry `TUNE, TRIGGER trigger, pp #sqrt{s} = 13.6 TeV`. The trigger is
// now a column header and the beam energy is in the information block, so the
// only fact left that varies panel by panel within a column is the tune. It
// is drawn inside the frame, top-left, small and without a box, so each panel
// still identifies itself when read alone.
//
// THE CLEARANCE IS MEASURED AGAINST THE DRAWN POINTS, not assumed, which is
// the rule FIG-1B established for the correlation legend. The label box is
// converted from pad NDC into user coordinates using the pad's OWN configured
// window -- the same numbers `SetRangeUser` was given -- and every drawn point
// and error bar is tested against it. Top-left is tried first because that is
// where a reader looks; top-right is the fallback, and the choice is echoed so
// a render log says which corner each panel used.
//
// IF NEITHER CORNER IS CLEAR the label is still drawn, top-left, and the run
// says so on stderr. A panel with no identification is worse than one whose
// label is close to a point, and a silent drop would be invisible until the
// figure was read.
constexpr Double_t kPanelLabelInsetX = 0.025;
constexpr Double_t kPanelLabelInsetY = 0.030;
constexpr Double_t kPanelLabelBoxWidth = 0.30;
constexpr Double_t kPanelLabelBoxHeight = 0.11;

std::vector<TH1D*> DrawnHistogramsOnPad(TPad* pad)
{
    std::vector<TH1D*> drawn;
    if (!pad) { return drawn; }
    TIter next(pad->GetListOfPrimitives());
    while (TObject* primitive = next()) {
        TH1D* hist = dynamic_cast<TH1D*>(primitive);
        if (!hist) { continue; }
        if (TString(hist->GetName()).Contains("Template")) { continue; }
        drawn.push_back(hist);
    }
    return drawn;
}

bool PanelLabelRegionIsClear(const std::vector<TH1D*>& drawn,
                             Double_t yMinAxis, Double_t yMaxAxis, bool logy,
                             Double_t x0Ndc, Double_t y0Ndc,
                             Double_t x1Ndc, Double_t y1Ndc)
{
    if (drawn.empty() || !gPad) { return true; }
    ...
    auto userY = [&](Double_t ndc) {
        ...
        if (!logy) { return yMinAxis + f * (yMaxAxis - yMinAxis); }
        const Double_t lo = std::log10(std::max(yMinAxis, 1e-300));
        const Double_t hi = std::log10(std::max(yMaxAxis, 1e-300));
...
void DrawPanelLabel(TPad* pad, const std::string& label,
                    Double_t yMinAxis, Double_t yMaxAxis, bool logy)
{
    if (!pad || label.empty()) { return; }
    pad->cd();
    const std::vector<TH1D*> drawn = DrawnHistogramsOnPad(pad);
    const Double_t lm = pad->GetLeftMargin();
    const Double_t rm = pad->GetRightMargin();
    const Double_t tm = pad->GetTopMargin();

    const Double_t topY = 1.0 - tm - kPanelLabelInsetY;
    const Double_t leftX = lm + kPanelLabelInsetX;
    const Double_t rightX = 1.0 - rm - kPanelLabelInsetX - kPanelLabelBoxWidth;

    Double_t x = leftX;
    const char* corner = "top-left";
    if (!PanelLabelRegionIsClear(drawn, yMinAxis, yMaxAxis, logy,
                                 leftX, topY - kPanelLabelBoxHeight,
                                 leftX + kPanelLabelBoxWidth, topY)) {
        if (PanelLabelRegionIsClear(drawn, yMinAxis, yMaxAxis, logy,
                                    rightX, topY - kPanelLabelBoxHeight,
                                    rightX + kPanelLabelBoxWidth, topY)) {
            x = rightX;
            corner = "top-right";
        } else {
            corner = "top-left-CROWDED";
            std::cerr << "WARNING: panel label '" << label << "' on pad "
                      << pad->GetName() << " has no clear corner; drawn "
                      << "top-left over the data band" << std::endl;
        }
    }

    TLatex* text = new TLatex();
    text->SetNDC();
    // The size is a placeholder: ApplyPublicationTextStyleToPad restyles every
    // TLatex it finds on an assembled canvas, so this label takes the final
    // metric from the composite width rather than from the mini pad's.
    text->SetTextFont(43);
    text->SetTextSize(20);
    text->SetTextAlign(13);
    text->DrawLatex(x, topY, label.c_str());
    std::cout << "PANEL_LABEL pad=" << pad->GetName() << " text=" << label
              << " corner=" << corner << std::endl;
}

// ============================================================================
// THE THREE CANVAS-LEVEL LABELS (ruling R46, refined by L1). All three are
// drawn on the GLOBAL canvas after the mini pads have been re-parented and
// after the publication text pass, so they take the final canvas width and
// sit over the assembled figure.
//
// THE ANCHOR IS THE ONE THE SINGLE-LINE DECLARATION ALREADY USED: 0.035 from
// the left, 0.985 from the bottom. The block grows DOWNWARD from it, so the
// first line stays exactly where the delivered figures' declaration was and
// old-versus-new comparison has a fixed reference.
// ============================================================================

// THE BAND ABOVE THE PADS HOLDS THREE THINGS IN THREE ROWS, and they do not
// share a row. The generator stops every mini pad at 0.90
// (`CANVAS_LABEL_BAND_TOP`), which leaves 0.10 of the height here.
//
// WHY NOT TWO OF THEM SIDE BY SIDE. The block's coverage line reaches 0.46 of
// the width on V-EXTREMES and further on the closure canvas, and an extremes
// canvas's legend measures 0.66 of the width with its five entries. Replayed
// on one row the two overprint. The rows below are measured against the
// widest case, not the reference figure -- the rule the architect made
// binding after the FIG-1 text metric was measured on G1 alone.
constexpr Double_t kInformationBlockX = 0.035;
constexpr Double_t kInformationBlockTopY = 0.995;

// Line pitch as a multiple of the text size. 1.35 gives a clear gap between
// two lines of the label metric without opening one the eye reads as a break.
constexpr Double_t kInformationBlockLinePitch = 1.35;

// The canvas legend takes the row below the block, right-aligned, laid out
// horizontally so it costs one row rather than a column of plot height (L1).
constexpr Double_t kCanvasLegendRightX = 0.985;
constexpr Double_t kCanvasLegendY = 0.947;

// Column headers take the row below the legend, immediately above the pads.
constexpr Double_t kColumnHeaderY = 0.905;

// `pad` may be the canvas itself, which is where every composite draws its
// block, or a sub-pad, which is what the correlation canvas needs: its four
// pads tile the whole canvas, so there is no band outside them to draw in and
// the block goes inside the first pad's frame, as G1's does.
void DrawInformationBlock(TVirtualPad* pad,
                          const std::vector<std::string>& lines,
                          Double_t canvasWidthPx,
                          Double_t x = kInformationBlockX,
                          Double_t topY = kInformationBlockTopY)
{
    if (!pad || lines.empty()) { return; }
    pad->cd();
    const Int_t pixels =
        PublicationTextPixels(kPublicationLabelFraction, canvasWidthPx);
    // The pitch is in the PAD's own NDC, so the pixel size is divided by the
    // pad's height in pixels, not the canvas's. On the canvas itself
    // GetAbsHNDC() is 1 and the two are the same number.
    const Double_t canvasHeightPx =
        pad->GetWh() > 0 ? static_cast<Double_t>(pad->GetWh()) : canvasWidthPx;
    const Double_t padHeightPx = canvasHeightPx * pad->GetAbsHNDC();
    const Double_t step =
        kInformationBlockLinePitch * pixels / (padHeightPx > 0 ? padHeightPx : canvasHeightPx);
    for (std::size_t line = 0; line < lines.size(); ++line) {
        TLatex* text = new TLatex();
        text->SetNDC();
        text->SetTextFont(43);
        text->SetTextSize(pixels);
        text->SetTextAlign(13);  // left, top: the anchor is the line's top
        text->DrawLatex(x, topY - step * static_cast<Double_t>(line),
                        lines[line].c_str());
        // RUN-N4's style-delta proof reads this line, so it stays and every
        // line of the block is echoed rather than only the first.
        std::cout << "AXIS_DECLARATION " << lines[line] << std::endl;
    }
}

void DrawColumnHeaders(TCanvas* canvas,
                       const std::vector<ColumnHeader>& headers,
                       Double_t canvasWidthPx)
{
    if (!canvas || headers.empty()) { return; }
    canvas->cd();
    const Int_t pixels =
        PublicationTextPixels(kPublicationTitleFraction, canvasWidthPx);
    for (const ColumnHeader& header : headers) {
        if (header.text.empty()) { continue; }
        TLatex* text = new TLatex();
        text->SetNDC();
        text->SetTextFont(43);
        text->SetTextSize(pixels);
        text->SetTextAlign(21);  // centred on the column, sitting on its baseline
        text->DrawLatex(0.5 * (header.xMin + header.xMax), kColumnHeaderY,
                        header.text.c_str());
        std::cout << "COLUMN_HEADER x=[" << header.xMin << "," << header.xMax
                  << "] " << header.text << std::endl;
    }
}

// The canvas legend is built from TUNE NAMES, and every visual attribute is
// resolved from plotting/TunePlotStyle.h. A configuration that wanted to
// recolour a tune would have to edit the header, which is where O3 put that
// decision.
void DrawCanvasLegend(TCanvas* canvas,
                      const std::vector<CanvasLegendEntry>& entries,
                      Double_t canvasWidthPx)
{
    if (!canvas || entries.empty()) { return; }
    canvas->cd();
    const Int_t pixels =
        PublicationTextPixels(kPublicationLabelFraction, canvasWidthPx);
    // Width is taken from the entries themselves so the box ends where the
    // text does: a fixed width either clips the longest entry or leaves a gap.
    Double_t widest = 0.0;
    for (const CanvasLegendEntry& entry : entries) {
        const std::string text =
            entry.kind == "tune" ? entry.tune : entry.label;
        TLatex measure;
        measure.SetTextFont(43);
        measure.SetTextSize(pixels);
        UInt_t width = 0, height = 0;
        measure.SetText(0.0, 0.0, text.c_str());
        measure.GetBoundingBox(width, height);
        if (width > widest) { widest = width; }
    }
    // Each horizontal cell holds a marker sample and its text. The sample
    // takes about two text heights; the rest is the widest label.
    const Double_t cellPx = widest + 3.0 * pixels;
    const Double_t totalNdc =
        cellPx * static_cast<Double_t>(entries.size()) / canvasWidthPx;
    const Double_t heightNdc =
        1.6 * pixels / (canvas->GetWh() > 0 ? canvas->GetWh() : canvasWidthPx);
    Double_t x0 = kCanvasLegendRightX - totalNdc;
    if (x0 < 0.0) { x0 = 0.0; }

    TLegend* legend = new TLegend(x0, kCanvasLegendY - heightNdc,
                                  kCanvasLegendRightX, kCanvasLegendY);
    legend->SetBorderSize(0);
    legend->SetFillStyle(0);
    legend->SetNColumns(static_cast<Int_t>(entries.size()));
    // The marker samples need a histogram to hang on. They are drawn nowhere
    // and are owned by the legend's own list through AddEntry.
    for (const CanvasLegendEntry& entry : entries) {
        TH1D* sample = new TH1D(
            Form("canvasLegendSample_%s_%s_%s", canvas->GetName(),
                 entry.kind.c_str(),
                 (entry.kind == "tune" ? entry.tune : entry.fill).c_str()),
            "", 1, 0.0, 1.0);
        sample->SetDirectory(nullptr);
        sample->SetLineStyle(HadronizationPlotStyle::kTuneLineStyle);
        if (entry.kind == "tune") {
            const Color_t colour = HadronizationPlotStyle::TuneColor(entry.tune);
            sample->SetLineColor(colour);
            sample->SetMarkerColor(colour);
            sample->SetMarkerStyle(HadronizationPlotStyle::TuneMarker(entry.tune));
            legend->AddEntry(sample, entry.tune.c_str(), "lep");
        } else if (entry.kind == "class_fill") {
            // THE FILL CONVENTION IS SHOWN ON A NEUTRAL MARKER. Colour and
            // shape are already spent on the tune, so the class entry must not
            // claim either: it takes MONASH's circle in black, open or filled,
            // which is the shape the reader has just been told means MONASH.
            // That is the least misleading of the three tune shapes to borrow,
            // and the entry's own text says what the fill means.
            const Style_t filled = HadronizationPlotStyle::kMonashMarker;
            sample->SetLineColor(kBlack);
            sample->SetMarkerColor(kBlack);
            sample->SetMarkerStyle(
                entry.fill == "open"
                    ? HadronizationPlotStyle::OpenTuneMarker(filled)
                    : filled);
            legend->AddEntry(sample, entry.label.c_str(), "p");
        } else {
            throw std::runtime_error(
                "unknown canvas_legend kind '" + entry.kind + "'");
        }
        std::cout << "CANVAS_LEGEND kind=" << entry.kind << " text="
                  << (entry.kind == "tune" ? entry.tune : entry.label)
                  << std::endl;
    }
    legend->Draw();
    // The publication pass has already run on this canvas, so this legend --
    // created after it -- takes the metric here, entries included.
    ApplyPublicationLegendStyle(legend, canvasWidthPx);
}
...
CONFIGS readConfig(const char* configurations) {
   ...
   std::map<std::string, std::vector<TriggerAssociateOSandSS>> charmConfigs;
    ...
    for (const auto& configPair : config["canvases_to_be_drawn"]) {
        ...
        // A PANEL THAT DRAWS NO LEGEND CARRIES NO RECTANGLE (ruling L1). The
        // generated configurations no longer write these four keys at all, so
        // they are read with the macro's own "no legend" sentinel as the
        // default instead of with the const `operator[]`, which asserts on a
        // missing key. The frozen base and the hand-maintained files still
        // carry them and are unaffected.
        pair.xMinLegend = configPair.value("x_min_legend", -1.0);
        pair.xMaxLegend = configPair.value("x_max_legend", -1.0);
        pair.yMinLegend = configPair.value("y_min_legend", -1.0);
        pair.yMaxLegend = configPair.value("y_max_legend", -1.0);
        // THE IN-FRAME ROW LABEL (R46 item 4): the tune alone, or empty on a
        // panel that does not vary by tune. Optional for the same reason.
        pair.panelLabel = configPair.value("panel_label", std::string());
        std::map<std::string, std::string> legendEntriesMap;
    ...
    for (const auto& configPair : config["global_canvases_to_be_drawn"]) {
        ...
        pair.xSizeCanvas = configPair["x_size_canvas"].get<Double_t>();
        pair.ySizeCanvas = configPair["y_size_canvas"].get<Double_t>();
        // BOTH CANVAS-LEVEL KEYS ARE OPTIONAL (R46, L1). The generator writes
        // them on every global it emits; the frozen base and the four
        // hand-maintained configurations carry neither and render as before,
        // with no header and no canvas legend. `value` is used rather than
        // `operator[]` for exactly that reason -- the const `operator[]`
        // asserts on a missing key.
        for (const auto& header : configPair.value("column_headers", json::array())) {
            ColumnHeader column;
            column.text = header.at("text").get<std::string>();
            column.xMin = header.at("x_min").get<Double_t>();
            column.xMax = header.at("x_max").get<Double_t>();
            pair.columnHeaders.push_back(column);
        }
        for (const auto& entry : configPair.value("canvas_legend", json::array())) {
            CanvasLegendEntry legendEntry;
            legendEntry.kind = entry.at("kind").get<std::string>();
            legendEntry.tune = entry.value("tune", std::string());
            legendEntry.fill = entry.value("fill", std::string());
            legendEntry.label = entry.value("label", std::string());
            pair.canvasLegend.push_back(legendEntry);
        }
    ...
    configs_from_json.axisDeclaration = ParseInformationBlock(config);
...
YieldsAndErrorsMap calculateYieldsVector(CONFIGS configs_from_json, const char* FLAVOUR, TCanvas* cAngularCorrelations) {
    ...
    // THE COLUMN HEADERS OF THIS CANVAS (R46 item 3). They are captured from
    // the same dispatch that chooses each trigger's pad, so a header cannot
    // name a column the canvas does not draw. Drawn once, after the style
    // pass, in the band the panel titles used to occupy.
    std::string correlationMesonHeader;
    std::string correlationBaryonHeader;
    ...
    if (DRAW_CORRELATION_PLOTS) {
        if (strcmp(FLAVOUR, "CHARM") == 0) {
            ...
            pCharmMeson      = createMiniPad("pCharmMeson",     0.00,kCorrelationRowSplit,0.50,kCorrelationRowTop);
            pCharmBaryon     = createMiniPad("pCharmBaryon",    0.50,kCorrelationRowSplit,1.00,kCorrelationRowTop);
            pCharmMesonSub   = createMiniPad("pCharmMesonSub",  0.00,0.00,0.50,kCorrelationRowSplit);
            pCharmBaryonSub  = createMiniPad("pCharmBaryonSub", 0.50,0.00,1.00,kCorrelationRowSplit);
            pCharmMeson->cd(); gPad->SetLogy();
            pCharmBaryon->cd(); gPad->SetLogy();
        }
        if (strcmp(FLAVOUR, "BEAUTY") == 0) {
            cAngularCorrelations->cd();
            pBeautyMeson      = createMiniPad("pBeautyMeson",     0.00,kCorrelationRowSplit,0.50,kCorrelationRowTop);
            pBeautyBaryon     = createMiniPad("pBeautyBaryon",    0.50,kCorrelationRowSplit,1.00,kCorrelationRowTop);
            pBeautyMesonSub   = createMiniPad("pBeautyMesonSub",  0.00,0.00,0.50,kCorrelationRowSplit);
            pBeautyBaryonSub  = createMiniPad("pBeautyBaryonSub", 0.50,0.00,1.00,kCorrelationRowSplit);
                ...
                for (Int_t k=0; k<nDependencies; k++) {
                    ...
                    if (drawThisCorrelation) {
                        ...
                        if (fileNamesOSandSS.OS ==
                            ...
                            // `#bar#Lambda` without braces was the spelling
                            // here; every other site writes `#bar{#Lambda}`.
                            // Titles-off silences this string, but the column
                            // header below is built from the same notation and
                            // a malformed one would reach the figure.
                            title = "#bar{#Lambda}_{b}^{0} trigger";
                            correlationBaryonHeader = title.Data();
                        }
                        if (!padOSSS || !padSub) {
                            delete hSub;
                        ...
                        hDPhiOS->GetYaxis()->SetRangeUser(
                            kCorrelationYieldMin, kCorrelationYieldMax);
                        hDPhiOS->SetStats(0);
                        hDPhiSS->SetStats(0);
                        ...
                        // ONE LEGEND FOR THE CANVAS, IN THE FIRST PANEL
                        // (ruling L1). OS and SS are the same convention in
                        // both top panels, so the delivered figures stated it
                        // twice. L1 asks for it once, on the global canvas and
                        // outside every pad -- and this canvas has no outside:
                        // its four pads tile the full area, so a band above
                        // them would have to be taken out of the plots. The
                        // convention is therefore named once IN the first
                        // panel, which is R46 item 5's own wording and the
                        // scheme G1 uses. This is the session's one documented
                        // departure from "outside every pad", and its reason
                        // is geometric, not preference.
                        //
                        // Place the legend where the data is not. Upper right
                        // first, because that is the reviewed position; upper
                        // left when the away-side peak reaches into it. The
                        // information block takes the upper LEFT of this same
                        // panel, so a legend forced left would collide with it
                        // -- measured and reported if it ever happens.
                        const bool firstPanelOfCanvas =
                            (padOSSS == pCharmMeson) || (padOSSS == pBeautyMeson);
                        ...
                        if (!PadRegionIsClear(drawnPair, 0.55, kLegendTop - kLegendH,
                            ...
                            legX0 = 0.16;
                            if (firstPanelOfCanvas) {
                                std::cerr << "WARNING: the correlation legend "
                                             "moved to the upper LEFT, where "
                                             "the information block sits; "
                                             "check the crop" << std::endl;
                            }
                        }
                        if (firstPanelOfCanvas) {
    ...
    if (DRAW_CORRELATION_PLOTS) {
        ...
        // THE COLUMN HEADERS REPLACE THE PANEL TITLES (R46 item 3), in the
        // band those titles occupied: the top pads' 0.10 top margin, which is
        // 0.05 of the canvas and empty once the titles are off. Drawn after
        // the style pass so they take the final metric, and centred on each
        // column's own half of the canvas.
        {
            const Double_t width =
                static_cast<Double_t>(cAngularCorrelations->GetWw());
            // THE BLOCK THIS CANVAS NEVER HAD (R46 item 2). It is written
            // straight from here and never enters the global-canvas loop, so
            // the delivered G2 and G3 state no generator, no system and no
            // beam energy anywhere on the figure.
            DrawInformationBlock(cAngularCorrelations,
                                 configs_from_json.axisDeclaration, width);
            std::vector<ColumnHeader> headers;
            // The headers span each column's FRAME, not its pad: these pads
            // carry an 0.18 left margin, so the pad centre sits left of the
            // plot the header names.
            const Double_t leftInset = 0.18 * 0.50, rightInset = 0.03 * 0.50;
            if (!correlationMesonHeader.empty()) {
                headers.push_back({correlationMesonHeader,
                                   0.00 + leftInset, 0.50 - rightInset});
            }
            if (!correlationBaryonHeader.empty()) {
                headers.push_back({correlationBaryonHeader,
                                   0.50 + leftInset, 1.00 - rightInset});
            }
            DrawColumnHeaders(cAngularCorrelations, headers, width);
        }
...
TPad* drawBalancingPlots(CONFIGS configs_from_json, const char* FLAVOUR, YieldsAndErrorsMap mapYieldsAndErrors) {
    ...
    //
    // This is where `SetMoreLogLabels()` used to be called. It asks ROOT to
    // label the intermediate ticks as well, and it was added because a window
    // spanning less than one decade boundary gets a single labelled tick. At
    // the publication text metric the cure is worse: a four-decade axis then
    // prints `2x10^-1` immediately above `10^-1` and repeats the pair at every
    // decade, and the owner's crop shows the two nearly touching. G9's axis,
    // in this repository, labels decades only and is the reference.
    //
    // THE SHORT-RANGE CASE IS ANSWERED BY THE RANGE, NOT BY MORE LABELS. Two
    // configurations still span under three decades -- V-EXTREMES at 1.9 and
    // the V-CORRELATIONS composite at 1.6 -- so they label two decades. The
    // FIG-1D report states that and proposes the floors that would give three;
    // it does not put the clutter back to hide a range decision.
    //
    // Minor ticks are untouched and stay unlabelled, which is what carries the
    // reading between decades.
    ...
    // behind. Its clearance is measured against the points this function has
    // just drawn.
    DrawPanelLabel(cMiniPad, canvasConfigs.panelLabel,
                   canvasConfigs.yMinAxis, canvasConfigs.yMaxAxis,
                   canvasConfigs.setLogy);
...
int improvedPlotting_THnSparse(const char* configuration) {
    ...
    for (const auto& globalCanvasConfig : vGlobalCanvasConfigs) {
        ...
        // THE CANVAS STATES ITS IDENTIFICATION HERE, AND ONLY HERE (R46).
        // All three are drawn after the style pass so they sit over the
        // assembled pads and take their own metric from the final width.
        DrawInformationBlock(globalCanvas, configs_from_json.axisDeclaration,
                             globalCanvasConfig.xSizeCanvas);
        DrawColumnHeaders(globalCanvas, globalCanvasConfig.columnHeaders,
                          globalCanvasConfig.xSizeCanvas);
        DrawCanvasLegend(globalCanvas, globalCanvasConfig.canvasLegend,
                         globalCanvasConfig.xSizeCanvas);
