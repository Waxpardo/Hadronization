...
ARTIFACT="${HADRONIZATION_SPECIES_ARTIFACT:-${CHECKOUT}/AnalysisScripts/species_ordinals_v2.json}"
DECAY_MAP="${HADRONIZATION_DECAY_MAP:-${CHECKOUT}/AnalysisScripts/decay_parent_map_v2.json}"
...
MAN=$R/manifest.txt
{
  ...
  echo "# reader_sha=$(sha256sum "$READER" | awk '{print $1}')"
  echo "# artifact_sha=$(sha256sum "$ARTIFACT" | awk '{print $1}')"
  echo "# map_v2_sha=$(sha256sum "$DECAY_MAP" | awk '{print $1}')"
  echo "# registry_sha=$(sha256sum "$PAIR_REGISTRY" | awk '{print $1}')"
