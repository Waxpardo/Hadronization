#!/usr/bin/env python3
"""Compare a per-species subset table against the parent it was drawn from.

WHY THIS EXISTS. On 2026-08-11 an anchor extraction agreed with its parent to
better than 0.02 percentage points on every aggregate the project checked --
totals, species shares, the second-branch number, the map-v2 table -- while
carrying a single bin that was ~10 sigma wrong. That bin produced a confidently
stated -7.4 sigma physics result which better data contradicted the same day
(docs/ERROR_RECORD.md E4).

**Aggregate agreement is not bin agreement.** Four aggregate checks passed on a
defective dataset. This checks the bins.

THE NULL IS A REQUIRED ARGUMENT, WITH NO DEFAULT. There are two, they answer
different questions, and neither is right for both callers -- so neither caller
is allowed to inherit one silently. A default is exactly what encodes a choice
nobody made.
