...
The result is an upper bound because the map records only dominant products.
It cannot identify a non-dominant channel that reaches the same ground state.
...
#
# Version 1 mapped antiparticle parents to particle daughters.
# Keep its values so the tool rejects that defective table explicitly.
...
def species_level_residual(v2: dict, weight: dict[int, float], total: float):
    ...
    share of the total moves under trigger-level deduplication.
