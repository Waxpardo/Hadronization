...
class CombinationPolicyRequired(Exception):
    """Raised when a combination is asked to make a choice that is not the
    tool's to make.

    Both open questions were RULED by the owner on 2026-08-18 (pre-registration
    OWNER AMENDMENT A1 and A2), before any variation delta existed. The flags
    below are still required rather than defaulted: a caller must name the
    policy it is applying, so a future change of policy is visible at the call
    site instead of buried in a default.
    """


# Ruling A1: contribution is max(|D|, SEM(D)), continuous, no threshold cliff.
...
# Ruling A2: S6/A2 is on the M1..M5 partition and is never summed into per-class
# c1..c11 totals. Named here so the exclusion is a constant, not a caller's
# string literal that could be misspelled into silence.
