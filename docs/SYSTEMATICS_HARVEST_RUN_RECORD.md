...
From `docs/MERGE_V3_BAND_VALIDATION.md`, where the central campaign's timings
...
`docs/PUBLICATION_EXPORT_EXCLUSIONS.md` now exists, seeded with
...
Commit **`b598ce7`** installs `docs/writing_standard/` and
`tools/prose_check.py`. **The other active branch cherry-picks `b598ce7` at its
next session.**

The install brief asked for that note in `MERGE_CHECKLIST`. That file is
`docs/MERGE_CHECKLIST.md` on branch `freeze-contract-fix`, which the figure
...
`tools/statistical_robustness.py` is 2761 lines. `REPO_FILE_CENSUS.md` counts 25 references to it. It carries its own test
...
`docs/systematics_results_20260819/PER_CATEGORY_DELTAS.md`.
...
`docs/PROGRESS_PROBE_METHOD.md` **Correction 10 — a gate on the request cannot
...
[`docs/NIKHEF_CLEANUP_PLAN.md`](NIKHEF_CLEANUP_PLAN.md) maps all of
...
not a central value"* (`STATE.md` PENDING 8).
...
cost of a merge at this scale is in `docs/MERGE_V3_BAND_VALIDATION.md` — 15 legs
...
`STATE.md` carries the merge-readiness assessment; this section carries what the
