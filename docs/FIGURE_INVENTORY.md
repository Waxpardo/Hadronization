...
| E5 | ⚠ **check first — §6.1** |
| **mandatory cross-check** | the quantities overlap `docs/THREE_TUNE_CENTRAL_TABLE.md` and `MONASH_CENTRAL_TABLE.md`. The drawn integrated values must reproduce the published fractions; **A2's per-class systematic** (`docs/a2_results_20260813/`) applies to the multiplicity-differential panels and must be quoted per class, never integrated |
...
| `configuration_multiplicity.json` (legacy) | 12 — but **two-tune** |
...
|---|---|
| `figures/MultiplicityDistributionPercentileBoundaries_{CHARM,BEAUTY}_counts.pdf` | **figure 4** (`MultiplicitySpectrum_Shared_shape`) — one *shared* event spectrum replaces the flavour-duplicated pair, and `plotting/paper/figures/fig3_multiplicity_classes.svg` carries the class axis |
| the same two files duplicated inside `figures/Iñaki plots/` | as above — they are byte-duplicates of the top-level pair |
| `Iñaki plots/MinimumBiasSpeciesResolvedSpectraCompareTunes_{Charm,Beauty}_27-03-2026.png` | `plotting/paper/figures/fig1_species_decomposition.svg` |
...
MB percentiles, matching `plotting/paper/make_paper_figures.py`'s
`boundary_percentiles()`. **Figure 4 is not rendered until then** — rendering it
...
Five have dispositions in `REMOVALS.md` / `REPO_FILE_CENSUS.md`:
...
- the documented version **is** preserved on this branch, byte-identical
  (`attic/plotting/B_Balancing_GeneralPlotting.C`, sha `c41b52dc5440…` on both).
...
> | documented version preserved | **byte-identical**, `attic/plotting/B_Balancing_GeneralPlotting.C`, sha `c41b52dc5440…` on both main@`11884cf` and this branch |
>
...
**Item 9 — the coverage claim.** The legacy canvas is `c2x3_yields`
(`attic/plotting/B_Balancing_GeneralPlotting.C:1370`), "balancing yields for
