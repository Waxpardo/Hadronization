...
propagates **no** systematic uncertainty (`STATE.md`, *NOT PLANNED*: "PDF and
...
- **The tune bundle is the measurement, not a systematic.** MONASH / JUNCTIONS /
  ...
  alone — is documented in `STATE.md` and is **not** a systematic either. It is
...
merge, and `STATE.md` PENDING #5 records the checkout advance as **still
...
the campaign is recorded COMPLETE, and the checkout advance in `STATE.md` PENDING
...
1. **The production worker could not run at all from the restructured tree.**
   ...
   (`RENAMES.md` §1.1). It resolves to `<base>/generation/submit` and every job
...
is not comfortable. Registered consequences: `STATE.md` PENDING #7 (disk
consolidation, `docs/NIKHEF_DISK_INVENTORY.md` §7) becomes load-bearing rather
