...
> `dd502a10…` in `EXTRACTION_CONVENTIONS.md`, `MERGED_CONVENTION_TABLES.md` and
...
> not in a surprise.** See `docs/RESTRUCTURE_PLAN.md` §4.2.
...
|---|---|---|---|
| **G5** | `decay_parent_map_v1.json` (**HISTORY — defective**) | `a67e8ae5f853689c010e991859242a77b913787dd30ab3d4c1b68bc05758c00c` | `e343fd8872f9742b84ae3dce5085071c59eb289c1841faef3fff754effadcccb` |
...
`docs/MAP_V1_CONJUGATION_BUG.md` and `ERROR_RECORD.md` **E1** are about; deleting
...
**Current, from merged weights — `docs/MERGED_CONVENTION_TABLES.md`**, computed
...
`MONASH_CENTRAL_TABLE.md` §4a** (full check table: `RESTRUCTURE_PLAN.md` §1.3):
...
> proposed in `HANDOFF_20260812_prep_v3.md` §3.1 from reading the two tables
...
**Cross-check, independent path:** `MERGED_CONVENTION_TABLES.md` (v47) computed
...
in `ERROR_RECORD.md` E4, `MERGED_CONVENTION_TABLES.md` §5 and
...
about the −7.4 σ, are recorded in `ERROR_RECORD.md` E4. **Nothing there was
rewritten, and this annotation does not lift the quarantine.**

---

### 2.12 The paper figures

**Generated from committed tables by script, with no hand steps.**
`plotting/paper/make_paper_figures.py` (recipe **R13**).

| figure | sha256 | source |
|---|---|---|
| `fig1_species_decomposition.svg` | `316a7d99cadcd32e2807d65df6fc7b54049adf7cc44be1d19b77d2e63b89dcc9` | `anchors/merged_{monash,junctions,closepacking}_dedup`, central + ten blocks each |
| `fig2_m7_inclusive_shift.svg` | `ffec6a3da3a69fee2b410c7b330ee36d7bc2f101c92b175e22e33cca078dfb13` | `anchors/{m7_blocks,m7b_blocks}` via `aggregate_m7.py` |
| `fig3_multiplicity_classes.svg` | `e687b953cdb46eb3c9ee5abfb0ab2c89663f649c153fddb01d7642bc12c49ee2` | `anchors/b4_multiplicity_mb` (per-tune MB N_ch) |
...
**Figure 3 recomputes a published table rather than displaying it.** The
...
**Determinism is the load-bearing property**, so it is worth saying why the
figures are hand-emitted SVG rather than matplotlib output: matplotlib's bytes
move with its own version, with freetype, and with the installed font set, so a
pinned digest would fail on a different machine for reasons unrelated to the
physics. `plotting/paper/svgkit.py` has **no dependencies at all** and routes
every coordinate through one fixed-precision formatter, so the bytes are a pure
function of the input numbers and the source.

**Status: SELF-CHECKING** — `tests/test_paper_figures.py` runs the generator
twice and requires byte-identical output, recomputes the structural percentages
from the anchors and requires the figure to display them, and asserts that the
two labels which must never be dropped are present: fig 1's
*SELECTION-not-partition* caveat and fig 2's *INCLUSIVE-level* banner.

> **The figure that is deliberately missing.** The OS−SS observable versus
> multiplicity class has **no committed table** — it is what the A2 jobs produce
> (`docs/A2_PAIR_UNRESOLVED_RUN_RECORD.md`). `--figure ossvsmult` **fails
> closed** (R13b) rather than drawing a placeholder. A figure with invented data
...
| # | contract | where |
|---|---|---|
| **C-1** | PYTHIA 8.317, stock upstream tarball `sha256 1ae551d1…45adf`, unmodified, `-std=c++20` | `REPRODUCIBILITY.md` §1, `docs/WORKSPACE.md` |
| **C-2** | ROOT 6.30/01 ALICE CVMFS build | `REPRODUCIBILITY.md` §1 |
| **C-3** | producer binary sha256 `e54b27bb9e3fcfd42d70193e08e2eacf965cc5081eabb5c42a9971203f130659`, present in all 300 analysis outputs | `docs/PHASE4_SUBMISSION_CHECKLIST.md` B2 |
...
| **C-8** | the 34 breach partials were **moved, never deleted**, with a committed manifest | `docs/campaigns/HF_RUN3_V1_PARTIALS_ARCHIVE.md` |
...
must not orphan it. See `docs/NIKHEF_DISK_INVENTORY.md`.
...
| **R13b** | `plotting/paper/make_paper_figures.py --figure ossvsmult` | **refuses**, `NOT AVAILABLE` — the data does not exist yet and no placeholder is drawn |
...
| **N2** | merge timing / the 65–77 h band | the merge log **carries no timestamps**; every timing in `docs/MERGE_V3_BAND_VALIDATION.md` is reconstructed from filesystem mtimes and **cannot be re-derived once scratch is cleared** | ⚠ **the restructure must not clear merge scratch before the band is scored** |
...
`extraction/extract_species_decomposition.py:54` sets
`DECAY_MAP = REPO / "AnalysisScripts/decay_parent_map_v1.json"` and
`:203` uses it as the `--decay-map` default. **v1 is the map that does not
conjugate antiparticle decays** — ERROR_RECORD **E1**, 17.8 pp on D⁰.
...
>
> `docs/RESTRUCTURE_PLAN.md` §1.2 carries the scope, and flags the residual that
...
a67e8ae5f853689c010e991859242a77b913787dd30ab3d4c1b68bc05758c00c  decay_parent_map_v1.json
...
3. **Plotting outputs.** `plotting/PAPER_FIGURE_PROVENANCE.md` exists;
   ...
   **UNKNOWN** — see `docs/RESTRUCTURE_PLAN.md` §8.1 Q2.
...
| V-FULL png | `f2973994e803b138…` | *(not re-rendered)* | expected unchanged; verified at merge, item C/D of `MERGE_CHECKLIST.md` |
