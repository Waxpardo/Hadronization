...
> `ValidationReports/NCH_CALIBRATION_20260730.md`. It is **kept visible rather
...
> `ValidationReports/NCH_DECAY_POLICY_BIAS_8317.md`.
>
...
top-class-only observable** (`RELEASE_BLOCKERS.md`, G2).
...
**Why this is not a statistical compromise.** Discarding whole jobs that fail to
complete is unbiased. Events are independent draws, so conditioning a kept event
on "no *other* event in my job hung" carries no information about that event;
the surviving marginal distribution is exactly "events PYTHIA can generate",
which is the same conditioning any per-event abort would impose. What is **not**
safe is selecting *which* completed jobs to keep by completion order: fast jobs
are low-multiplicity, so first-N-to-finish would bias multiplicity and every
observable correlated with it. Keep all finishers, or select by pre-assigned
logical ID.
...
every other. `ValidationReports/PYTHIA_JUNCTION_HANG_20260731.md` carries the
...
**Evidence.** `ValidationReports/PTHAT_MULTIPLICITY_SCAN_8317.md`, measured on
...
error.** The per-tune empirical mapping (B4) remains the decider: what matters
is where each tune's percentile boundaries actually fall, not where a scaling
law says its screening scale sits.
...
corrected per-card text is **parked** in
`docs/REGISTRY_AND_MAPPING_PROPOSAL.md` rather than applied, because a card
...
At 2.0 the sample is minimum-bias-like and the percentile classes mean what
they say. Note the older -36 % figure quoted before this change was a PYTHIA
8.315 measurement; on 8.317 the same configuration gives -28.6 %.

**These numbers are MONASH only, and that is a live limitation, not a
footnote.** `ValidationReports/PTHAT_MULTIPLICITY_SCAN_8317.md:108-110` states
...
**Why this matters beyond tidiness.** Event activity is classified by
percentile *within each tune's own sample*. If the hard sample sits at a
different offset from minimum bias for each tune, then "0-20 %" is not the
same physical selection across tunes, and multiplicity dependence is central
to the study. Nothing currently measures this. Re-running
`Validation/CalibrateMultiplicityAgainstMinBias.C` for JUNCTIONS and
CLOSEPACKING at 2.0 would settle it, and it is cheap -- the MONASH scan was
20 000 events per point and about four minutes.
...
**Evidence.** `Makefile` `require-ordinal`; `RELEASE_BLOCKERS.md` B11 for the
...
**The consequence, stated rather than hidden.** **Scale and PDF systematics are
argued in the referee response, not computed.** For a tune-contrast paper whose
uncertainty axis is hadronisation that is defensible — **but it must be written
in the manuscript, not discovered at referee stage.** It is tracked as a
submission blocker alongside the wider systematics gap
(`RELEASE_BLOCKERS.md` B14), and the two decisions were signed together.
...
make check             # doctor + cards + cards-current + registry + 21 tests
```
...
1. **`pTHatMin` sensitivity is resolved for multiplicity, open for the
   observables.** The multiplicity half is settled by the threshold choice in
   Section 3.12: at 2.0 the sample is within 4.2 % of minimum bias, so the
   percentile classes are usable. Whether the *balance observable itself* moves
   with the threshold is not measured. `tools/evaluate_pthat_sensitivity.py`
   and `Validation/PTHatSensitivity.C` exist for this and have not been run.
   The predeclared statistics are unchanged: family size 192, Bonferroni
   critical value 5.797108070583989, `conservative_degrees_of_freedom = 9`.
   ...
   (`ValidationReports/PTHAT_MULTIPLICITY_SCAN_8317.md:62`).
