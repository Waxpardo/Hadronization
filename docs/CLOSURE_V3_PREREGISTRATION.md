...
| **C4** | The run resolves the object set from each file's **own** `analysis_schema` (`…_v3`), not the sidecar JSON, which still reads `…_v2` — see the dated private handoff, Section 5. **If the counts come back 1800/600, this is the first thing to check.** |
