...
| C8 | Trigger normalization comes only from `hTrKinematics`; OS and SS denominators must match | `plotting/improvedPlotting_THnSparse.C:3226-3230` | `config/pair_file_object_contract_v1.json` requires `hTrKinematics` and permits `hFlavourClosureSummary` | `tests/test_pair_object_contract.py` |
| C9 | The reported integrated yield covers the full stored angular axis | `plotting/improvedPlotting_THnSparse.C:3238-3240` | the pair-object contract declares no regional boundary | `tests/test_pair_object_contract.py` |
...
| C11 | Eleven percentile classes, resolved independently per tune | `plotting/improvedPlotting_THnSparse.C:1955-1957` | `config/multiplicity_percentile_classes_v2.json`, `definition` | `tests/test_harvest_class_axis.py`; `tests/test_plot_reference_multiplicity_contract.py` |
| C12 | Pooled central value; SEM across ten blocks on nine degrees of freedom; nonlinear quantities formed inside blocks | `extraction/harvest_class_axis.py:113-119`; `tools/build_canonical_manifest.py:281-287` | `config/statistical_robustness_v1.json`, `method` | `tests/test_statistical_robustness.py`; `tests/test_decompose_exit_status.py` |
...
| G2, G3 | the V-CORRELATIONS canvas, `plotting/improvedPlotting_THnSparse.C:4239-4241` | `plotting/configuration_multiplicity_HF_RUN3_V1_VCORRELATIONS.json` | pending RUN-N |
