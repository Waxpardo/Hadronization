...
| C8 | Trigger normalization comes only from `hTrKinematics`; OS and SS denominators must match | `plotting/improvedPlotting_THnSparse.C:3286-3290` | `config/pair_file_object_contract_v1.json` requires `hTrKinematics` and permits `hFlavourClosureSummary` | `tests/test_pair_object_contract.py` |
| C9 | The reported integrated yield covers the full stored angular axis | `plotting/improvedPlotting_THnSparse.C:3298-3300` | the pair-object contract declares no regional boundary | `tests/test_pair_object_contract.py` |
...
| C11 | Eleven percentile classes, resolved independently per tune | `plotting/improvedPlotting_THnSparse.C:2006-2008` | `config/multiplicity_percentile_classes_v2.json`, `definition` | `tests/test_harvest_class_axis.py`; `tests/test_plot_reference_multiplicity_contract.py` |
...
| G2, G3 | the V-CORRELATIONS canvas, `plotting/improvedPlotting_THnSparse.C:4299-4301` | `plotting/configuration_multiplicity_HF_RUN3_V1_VCORRELATIONS.json` | pending RUN-N |
