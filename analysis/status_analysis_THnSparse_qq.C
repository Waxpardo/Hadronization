...
// v3 because the file now carries hFlavourClosureSpecies and its label
// objects. The pair-object contract is exact-match in both directions and is
// keyed on this string: v2 means six closure-checked content objects, v3 means
// seven. Bumping it is what tells every consumer which set to expect, so it
// MUST move in the same commit as the new object -- a file carrying the object
// while still declaring v2 would be rejected, correctly, by its own contract.
...
struct TriggerAccumulator {
  ...
  // The same observable resolved by species instead of by category. 33-49 % of
  // compensating heavy flavour lives in the categories the 6-bin axis cannot
  // distinguish; this axis names the hadron.
  THnSparseD* closureSpecies = nullptr;
...
// Closure sparse: (dphi, deta, assoc pT, Nch, LAST). The last axis labels each
// compensating hadron; the ground-state set is a labelled SUBSET of the closed
// sum rather than a separate, non-closing observable.
//
// TWO OBJECTS, ONE CONSTRUCTOR, on purpose. hFlavourClosure keeps the 6-bin
// heavyStateCategory axis; hFlavourClosureSpecies carries the 202-bin species
// ordinal instead. The species axis is validated by summing it by category and
// comparing against the category axis BIN FOR BIN, which is only meaningful if
// the other four axes are identical -- so they are built here once rather than
// written out twice and trusted to stay in step.
//
// It is a PARALLEL object, not a widening of hFlavourClosure, which stays
// byte-identical. hFlavourClosure carried 124,494 filled bins at 100 inputs and
// is the object whose merge cost produced a 49x step whose mechanism is still
// unknown; widening it in place could not be bounded.
THnSparseD* MakeClosureWithLastAxis(const char* name, const char* title,
                                    int lastBins, double lastMinimum,
                                    double lastMaximum,
                                    const char* lastAxisTitle) {
  const std::vector<double> ptEdges = PtEdges();
  const std::vector<double> deltaEtaEdges =
      InclusiveUniformEdges(100, -8.0, 8.0);
  int bins[5] = {100, 100, static_cast<int>(ptEdges.size()) - 1, 4096,
                 lastBins};
  double minimum[5] = {-M_PI / 2.0, -8.0, 0.0, -0.5, lastMinimum};
  double maximum[5] = {
      3.0 * M_PI / 2.0,
      std::nextafter(8.0, std::numeric_limits<double>::infinity()),
      7000.0, 4095.5, lastMaximum};
  auto* histogram = new THnSparseD(name, title, 5, bins, minimum, maximum);
  ...
  histogram->GetAxis(4)->SetTitle(lastAxisTitle);
  histogram->Sumw2();
  return histogram;
}

THnSparseD* MakeClosure(const char* name) {
  return MakeClosureWithLastAxis(
      name, "(dphi,deta,asPt,Nch,heavyStateCategory)", 6, -0.5, 5.5,
      "heavy state category");
}

THnSparseD* MakeClosureSpecies(const char* name) {
  return MakeClosureWithLastAxis(
      name, "(dphi,deta,asPt,Nch,speciesOrdinal)",
      Hadronization::kSpeciesOrdinalCount, -0.5,
      Hadronization::kSpeciesOrdinalCount - 0.5, "species ordinal");
}
...
int status_analysis_THnSparse_qq(
  ...
  try {
    ...
    for (const auto& definition : Hadronization::kPairDefinitions) {
      ...
      trigger.closureSpecies = MakeClosureSpecies("hFlavourClosureSpecies");
    ...
    for (Long64_t event = 0; event < chain.GetEntries(); ++event) {
      ...
      for (std::size_t triggerIndex = 0; triggerIndex < size; ++triggerIndex) {
        ...
        for (std::size_t closureIndex = 0; closureIndex < size;
          ...
          // FAIL-CLOSED (F6). This hadron carries net heavy flavour -- the
          // closureCharge != 0 test above established that -- so it MUST have
          // an ordinal. A sector-charged PDG absent from the table means the
          // table and the generator disagree about what PYTHIA can produce,
          // and the axis is then incomplete in a way no bin can express.
          // There is deliberately no overflow bin: an overflow bin is exactly
          // how 152 species became invisible, and reintroducing one here would
          // hide the same failure again.
          const int closurePdg = (*heavyPdg)[closureIndex];
          int speciesOrdinal = -1;
          if (!Hadronization::SpeciesOrdinalFor(closurePdg, speciesOrdinal)) {
            throw std::runtime_error(
                "species ordinal table has no entry for sector-charged PDG " +
                std::to_string(closurePdg) +
                " (q_c=" + std::to_string((*heavyQc)[closureIndex]) +
                ", q_b=" + std::to_string((*heavyQb)[closureIndex]) +
                "). The table is the complete admissible set and carries " +
                std::to_string(Hadronization::kSpeciesOrdinalCount) +
                " species at digest " +
                std::string(Hadronization::kSpeciesOrdinalDigest) +
                "; regenerate it from this campaign's heavy_stability_audit "
                "with tools/GenerateSpeciesOrdinals.C rather than adding an "
                "overflow bin.");
          }
          const double speciesValues[5] = {
              closureValues[0], closureValues[1], closureValues[2],
              closureValues[3], static_cast<double>(speciesOrdinal)};
          trigger.closureSpecies->Fill(speciesValues, closureWeight);
        }
    ...
    for (auto& pair : pairs) {
      ...
      trigger.closureSpecies->Write("hFlavourClosureSpecies");
      ...
      // F5 LEGIBILITY. The species axis is 202 anonymous integers without
      // these. The ordinal -> PDG map travels in the file itself, following
      // the associate_origin_category_labels precedent, so a reader can decode
      // the axis without this repository. The digest identifies WHICH table,
      // so two files can be compared without trusting their filenames.
      TObjString speciesOrdinalSchema(Hadronization::kSpeciesOrdinalSchema);
      speciesOrdinalSchema.Write("species_ordinal_schema");
      TObjString speciesOrdinalLabels(Hadronization::kSpeciesOrdinalLabels);
      speciesOrdinalLabels.Write("species_ordinal_labels");
      TObjString speciesOrdinalDigest(Hadronization::kSpeciesOrdinalDigest);
      speciesOrdinalDigest.Write("species_ordinal_digest");
