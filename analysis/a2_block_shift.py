...
# The analyzer requires a passing regression sentinel for the selected variation.
# The gate checks three identities:
#
#   1. The sentinel records PASS.
#   2. The sentinel digest belongs to a registered variation.
#   3. The requested variation resolves to that digest.
# Each variation needs its own sentinel, so evidence from one cannot certify another.
...
def default_sentinel(variation: str) -> Path:
    ...
    """
    return (REPO / "results/a2/20260813/sentinels" /
            f"a2_regression_pass_{variation}.json")
...
def main() -> int:
    ...
    ap.add_argument("--regression-sentinel", type=Path, default=None,
                    help="path to the regression-pass sentinel; defaults to "
                         ...
                         "a2_regression_pass_<variation>.json. "
