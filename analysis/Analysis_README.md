...
See [`../docs/REPRODUCIBILITY.md`](../docs/REPRODUCIBILITY.md) for the complete gate and
production sequence.
...
from an authorized HTCondor submit host:
...
Use `--submit` on the authorized submit host after inspection. Optional
...
Central values come from the complete sealed `N`-file union per tune. For
`HF_RUN3_V1`, that union contains 1,000 files of 100,000 successful events per
tune, and each of the ten disjoint blocks contains 100 files. Central values
are not the mean of block estimators. With `K=10`:
...
- excluded split-chain charm, beauty, and status reducers from the private development history;
...
Only the retained PUBLIC diagnostic remains runnable here; the excluded reducers are history, not dependencies. In particular,
